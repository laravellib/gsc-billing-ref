<template>
    <div class="container dave-template">
                                ...
                                <div class="col-md-11">
                                    <h5>
                                        <b v-show="!updateMeHeader"
                                            >CREATE FUEL HEADER</b
                                        >
                                        <b v-show="updateMeHeader"
                                            >UPDATE FUEL HEADER</b
                                        >
                                    </h5>
                                </div>
                                <div class="col-md-1">
        ...
        ></search-signatory>
        <search-fuelHeader
            v-on:rowClick="fuelHeaderClose($event)"
        ></search-fuelHeader>
        <search-fuelListModal
            v-on:rowClick="fuelListClose($event)"
        ></search-fuelListModal>
    </div>
</template>
...
import searchFUELHeader from "../search/SearchAllowance/SearchFUELHeader.vue";
import searchFUELListModal from "../search/SearchAllowance/SearchFUELList.vue";
export default {
    components: {
        "search-signatory": searchSignatory,
        "search-fuelHeader": searchFUELHeader,
        "search-fuelListModal": searchFUELListModal
    },
    data() {
        return {
            rows: [],
    ...
    methods: {
        ...
        getDetail() {
            if (this.header.FHID) {
                axios
                    .get("api/fuel", {
                        params: { getDtl: true, id: this.header.FHID }
                    })
                    .then(response => {
        ...
        getTotal() {
            if (this.header.FHID) {
                axios
                    .get("api/fuel", {
                        params: { getTotal: true, id: this.header.FHID }
                    })
                    .then(response => {
                        if (response.data.length > 0) {
        ...
        getMarkup() {
            axios
                .get("api/markup")
                .then(response => {
                    this.markup = response.data[0].FuelMarkUp;
        ...
        saveHdr() {
            ...
            this.$Progress.start();
            axios
                .post("api/fuelHdr", data)
                .then(response => {
                    if (response.data.success) {
                        this.header = Object.assign({}, this.formHeader);
                        if (response.data.id) {
                            this.header.FHID = response.data.id;
                        }
                        toast.fire({
                            icon: "success",
        ...
        deleteHdr() {
            ...
            swal.fire({
                ...
                if (result.value) {
                    this.$Progress.start();
                    axios
                        .delete(`api/fuelHdrDelete/${this.header.FHID}`)
                        .then(response => {
                            if (response.data.success) {
                                this.clearAll();
                                swal.fire(
        ...
        saveDtl() {
            ...
            this.$Progress.start();
            axios
                .post("api/fuelDtl", data)
                .then(response => {
                    if (response.data.success) {
                        if (response.data.id) {
                            this.header.FEDID = response.data.id;
                        }
                        toast.fire({
                            icon: "success",
                            title: response.data.message
        ...
        deleteDtl() {
            ...
            swal.fire({
                ...
                if (result.value) {
                    this.$Progress.start();
                    axios
                        .delete(`api/deleteFUELHDR/${this.dtl.FEDID}`)
                        .then(response => {
                            if (response.data.success) {
                                this.getDetail();
                                this.clearHeader("detail");
                ...
                .then(response => {
                    ...
                    if (response.data.success) {
                        this.reportData.url2 =
                            "api/reportFUEL?report=true&SOANo=" +
                                this.header.SOANo
                        ;
                    } else {
                    ...
                    .then(response => {
                        if (response.data.success) {
                            this.reportData.url1 =
                                ...
                                "&reportID=" + response.data.id + "&type=FUEL";
                        }
                        this.$Progress.finish();
                    })
                    .catch(function(error) {
