...
<script>
import locationRate from "./Liftruck-Location-Rate.vue";
export default {
  components: {
  ...
  methods: {
    addRateModal() {
      axios
        .get("liftruck/location_get_rate/" + this.form.id)
        .then(({ data }) => {
          if (data.data == null) {
            this.rate = 0;
          } else {
    ...
    updateData() {
      this.form
        .put("api/liftruck_location/" + this.form.id)
        .then(() => {
      ...
      swal
        ...
        .then(result => {
          if (result.value) {
            console.log(id);
            this.form
              .delete("api/liftruck_location/" + id)
              .then(() => {
    ...
    loadGolfcart() {
      axios.get("api/liftruck_location").then(({ data }) => {
