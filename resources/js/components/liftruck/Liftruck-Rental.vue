    ...
    <div class="row">
      ...
      <div class="container tab-content">
        <form @submit.prevent="createClient()">
          <div class="row mt-3">
            <div class="col-sm-4">
              <label for="refence">Rental #</label>
              <input
                type="text"
...
<script>
import searchDetail from "./Liftruck-PO-Search-Detail.vue";
import searchRequisition from "./Liftruck-Search-Rental.vue";
export default {
  components: {
  ...
  methods: {
    post_dmpi() {
      if (this.dtl.id == 0) {
        ...
        swal
          ...
          .then(result => {
            if (result.value) {
              axios.get("liftruck/req_post/" + this.dtl.id).then(({ data }) => {
    ...
    createClient() {
      if (this.dtl.status == "POSTED") {
        ...
        this.$Progress.start();
        axios
          .post("liftruck/req_client", this.dtl)
          .then(data => {
            toast.fire({
              icon: "success",
