...
export default {
  ...
  methods: {
    get_signatory(data) {
      ...
      this.$Progress.start();
      axios.get("rental_liftruck_soa/details/" + this.id).then(({ data }) => {
        var doc = new jsPDF();
        doc.setFont("courier");
        ...
        doc.save("liftruck_soa_" + this.soa_no + ".pdf");
      });
      ...
      if (this.id == "") {
        ...
        swal
          ...
          .then(result => {
            if (result.value) {
              axios
                .get("rental_liftruck_soa/collected/" + this.id)
                .then(({ data }) => {
                  this.review();
                });
            }
        ...
        swal
          ...
          .then(result => {
            if (result.value) {
              console.log(id);
              axios
                .get("rental_liftruck_soa/cancel/" + this.id)
                .then(({ data }) => {
                  this.review();
                });
            }
    ...
    review() {
      axios
        .get("rental_liftruck_soa/review/" + this.datefrom + "/" + this.dateto)
        .then(({ data }) => {
