<template>
    <div class="container dave-template">
        <div class="col-xs-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">
                        <b>Personal Protective Equipments Entry</b>
                    </h3>
        ...
        ></search-signatory>
        <search-ppeHeader
            v-on:rowClick="ppeHeaderClose($event)"
        ></search-ppeHeader>
        <search-ppeListModal
            v-on:rowClick="ppeListClose($event)"
        ></search-ppeListModal>
    </div>
</template>
...
import searchPPEHeader from "../search/SearchAllowance/SearchPPEHeader.vue";
import searchPPEListModal from "../search/SearchAllowance/SearchPPEList.vue";
export default {
    components: {
        "search-signatory": searchSignatory,
        "search-ppeHeader": searchPPEHeader,
        "search-ppeListModal": searchPPEListModal
    },
    data() {
        return {
            rows: [],
    ...
    methods: {
        ...
        getDetail() {
            if (this.header.PHID) {
                axios
                    .get("api/ppe", {
                        params: { getDtl: true, id: this.header.PHID }
                    })
                    .then(response => {
        ...
        getTotal() {
            if (this.header.PHID) {
                axios
                    .get("api/ppe", {
                        params: { getTotal: true, id: this.header.PHID }
                    })
                    .then(response => {
                        if (response.data.length > 0) {
        ...
        getMarkup() {
            axios
                .get("api/markup")
                .then(response => {
                    this.markup = response.data[0].PPEMarkUp;
        ...
        saveHdr() {
            ...
            this.$Progress.start();
            axios
                .post("api/ppeHdr", data)
                .then(response => {
                    if (response.data.success) {
                        this.header = Object.assign({}, this.formHeader);
                        if (response.data.id) {
                            this.header.PHID = response.data.id;
                            this.header.Status = "ACTIVE";
                        }
                        toast.fire({
                            icon: "success",
        ...
        deleteHdr() {
            ...
            swal.fire({
                ...
                if (result.value) {
                    this.$Progress.start();
                    axios
                        .delete(`api/ppeHdrDelete/${this.header.PHID}`)
                        .then(response => {
                            if (response.data.success) {
                                this.clearAll();
                                swal.fire(
            ...
            if (this.dtl.markup == 0) {
                ...
                if (this.dtl.markup) {
                    if (!isNaN(this.dtl.Qty)) {
                        ...
                        this.MarkUpAmount = this.$root.formatNumberCommaRound(
                            (this.dtl.markup / 100) *
                                this.$root.formatNumber(this.dtl.Price)
                        );
                    }
        ...
        saveDtl() {
            ...
            this.$Progress.start();
            axios
                .post("api/ppeDtl", data)
                .then(response => {
                    if (response.data.success) {
                        if (response.data.id) {
                            this.header.PEDID = response.data.id;
                        }
                        toast.fire({
                            icon: "success",
                            title: response.data.message
        ...
        deleteDtl() {
            ...
            swal.fire({
                ...
                if (result.value) {
                    this.$Progress.start();
                    axios
                        .delete(`api/deletePPEHDR/${this.dtl.PEDID}`)
                        .then(response => {
                            if (response.data.success) {
                                this.getDetail();
                                this.clearHeader("detail");
        ...
        generatePrint() {
            ...
            axios
                .get("api/reportPPE", {
                    params: { SOANo: this.header.SOANo, type: "PPE" }
                })
                .then(response => {
                    console.log(response.data);
                    if (response.data.success) {
                        this.reportData.url2 =
                            "api/reportPPE?report=true&SOANo=" +
                                this.header.SOANo + "&type=PPE"
                        ;
                    } else {
                    ...
                    .then(response => {
                        if (response.data.success) {
                            this.reportData.url1 =
                                ...
                                "&reportID=" + response.data.id + "&type=PPE";
                        }
                        this.$Progress.finish();
                    })
                    .catch(function(error) {
