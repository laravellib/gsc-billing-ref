<template>
    <div class="container dave-template">
                                ...
                                <div class="col-md-11">
                                    <h5>
                                        <b v-show="!updateMeHeader"
                                            >CREATE SUPPLIES HEADER</b
                                        >
                                        <b v-show="updateMeHeader"
                                            >UPDATE SUPPLIES HEADER</b
                                        >
                                    </h5>
                                </div>
                                <div class="col-md-1">
        ...
        ></search-signatory>
        <search-supHeader
            v-on:rowClick="supHeaderClose($event)"
        ></search-supHeader>
        <search-supListModal
            v-on:rowClick="supListClose($event)"
        ></search-supListModal>
    </div>
</template>
...
import searchSUPHeader from "../search/SearchAllowance/SearchSUPHeader.vue";
import searchSUPListModal from "../search/SearchAllowance/SearchSUPList.vue";
export default {
    components: {
        "search-signatory": searchSignatory,
        "search-supHeader": searchSUPHeader,
        "search-supListModal": searchSUPListModal
    },
    data() {
        return {
            rows: [],
    ...
    methods: {
        ...
        getDetail() {
            if (this.header.SHID) {
                axios
                    .get("api/sup", {
                        params: { getDtl: true, id: this.header.SHID }
                    })
                    .then(response => {
        ...
        getTotal() {
            if (this.header.SHID) {
                axios
                    .get("api/sup", {
                        params: { getTotal: true, id: this.header.SHID }
                    })
                    .then(response => {
                        if (response.data.length > 0) {
        ...
        getMarkup() {
            axios
                .get("api/markup")
                .then(response => {
                    this.markup = response.data[0].SupMarkUp;
        ...
        saveHdr() {
            ...
            this.$Progress.start();
            axios
                .post("api/supHdr", data)
                .then(response => {
                    if (response.data.success) {
                        this.header = Object.assign({}, this.formHeader);
                        if (response.data.id) {
                            this.header.SHID = response.data.id;
                        }
                        toast.fire({
                            icon: "success",
        ...
        deleteHdr() {
            ...
            swal.fire({
                ...
                if (result.value) {
                    this.$Progress.start();
                    axios
                        .delete(`api/supHdrDelete/${this.header.SHID}`)
                        .then(response => {
                            if (response.data.success) {
                                this.clearAll();
                                swal.fire(
        ...
        saveDtl() {
            ...
            this.$Progress.start();
            axios
                .post("api/supDtl", data)
                .then(response => {
                    if (response.data.success) {
                        if (response.data.id) {
                            this.header.SEDID = response.data.id;
                        }
                        toast.fire({
                            icon: "success",
                            title: response.data.message
        ...
        deleteDtl() {
            ...
            swal.fire({
                ...
                if (result.value) {
                    this.$Progress.start();
                    axios
                        .delete(`api/deleteSUPHDR/${this.dtl.SEDID}`)
                        .then(response => {
                            if (response.data.success) {
                                this.getDetail();
                                this.clearHeader("detail");
                ...
                .then(response => {
                    ...
                    if (response.data.success) {
                        this.reportData.url2 =
                            "api/reportSUP?report=true&SOANo=" +
                                this.header.SOANo
                        ;
                    } else {
                    ...
                    .then(response => {
                        if (response.data.success) {
                            this.reportData.url1 =
                                ...
                                "&reportID=" + response.data.id + "&type=SUPPLIES";
                        }
                        this.$Progress.finish();
                    })
                    .catch(function(error) {
