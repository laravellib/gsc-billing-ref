...
export default {
  ...
  methods: {
    get_signatory(data) {
      ...
      this.$Progress.start();
      axios.get("req_soa/details/" + this.id).then(({ data }) => {
        var doc = new jsPDF();
        doc.setFont("courier");
        ...
        doc.save("vanrental_soa_" + this.soa_no + ".pdf");
      });
    ...
    paid_soa() {
      if (this.id == "") {
        ...
        swal
          ...
          .then(result => {
            if (result.value) {
              axios.get("req_soa/collected/" + this.id).then(({ data }) => {
                this.review();
              });
            }
          });
      ...
      if (this.id == "") {
        ...
        swal
          ...
          .then(result => {
            if (result.value) {
              console.log(id);
              axios.get("req_soa/cancel/" + this.id).then(({ data }) => {
                this.review();
              });
            }
          });
    ...
    review() {
      axios
        .get("req_soa/review/" + this.datefrom + "/" + this.dateto)
        .then(({ data }) => {
