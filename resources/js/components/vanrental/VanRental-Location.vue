...
<script>
import locationRate from "./VanRental-Location-Rate.vue";
export default {
  components: {
  ...
  methods: {
    addRateModal() {
      axios
        .get("vanrental/location_get_rate/" + this.form.id)
        .then(({ data }) => {
          if (data.data == null) {
            this.rate = 0;
          } else {
    ...
    updateData() {
      this.form
        .put("api/vanrental_location/" + this.form.id)
        .then(() => {
      ...
      swal
        ...
        .then(result => {
          if (result.value) {
            console.log(id);
            this.form
              .delete("api/vanrental_location/" + id)
              .then(() => {
    ...
    loadGolfcart() {
      axios.get("api/vanrental_location").then(({ data }) => {
