<template>
    <div class="container dave-template">
        <div class="col-xs-12">
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">
                        <b>Create/Generate Incentives Transmittal</b>
                    </h3>
        ...
        <search-transmittal
            v-on:rowClick="transmittalClose($event)"
        ></search-transmittal>
        <search-incentivesHeader
            v-on:rowClick="incentivesHeaderClose($event)"
        ></search-incentivesHeader>
        <search-incentivesDetail></search-incentivesDetail>
    </div>
</template>

<script>
import searchTransmittal from "../search/SearchAllowance/SearchTransmittal.vue";
import searchIncentivesDetail from "../Incentives/ShowDetailModal.vue";
import searchIncentivesHeader from "../search/SearchAllowance/SearchIncentivesHeader.vue";
export default {
    components: {
        "search-transmittal": searchTransmittal,
        "search-incentivesDetail": searchIncentivesDetail,
    ...
    methods: {
                            ...
                            if (response.data.length > 0) {
                                swal.fire({
                                    ...
                                    if (result.value) {
                                        ...
                                        axios
                                            .post(
                                                "api/allowanceCreateTransmittal",
                                                data
                                            )
                                            .then(response => {
                                                if (response.data.success) {
                                                    ...
                                                    window.open(
                                                        "api/reportAllowanceTransmittal?report=true&transmittal_no=" +
                                                            this.transmittal_no + "&type=INCENTIVES"
                                                    );
                                                    this.transmittal_no = "";
                                                    this.generate();
                                                    this.getUser();
        ...
        printTransmittal() {
            axios
                .get("api/reportAllowanceTransmittal", { params: { searchTransmittalExists: true, transmittal_no: this.transmittal_no, type: "INCENTIVES" } })
                .then(response => {
                    if (response.data.success) {
                        window.open(
                            "api/reportAllowanceTransmittal?report=true&transmittal_no=" +
