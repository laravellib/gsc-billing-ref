<template>
    <div class="container dave-template">
                            ...
                            <div class="form-group">
                                ...
                                <b-input-group>
                                    ...
                                    <b-input-group-append>
                                        <b-button
                                            @click="searchIncentivesHeader(1)"
                            ...
                            >
                                ...
                                <tbody class="dave-tbody tbody-160">
                                    ...
                                    >
                                        ...
                                        <td>{{ item.LName }}, {{ item.FName }} {{ item.MName }} {{ item.ExtName }}</td>
                                        <td>{{ item.Amount | formatNumber }}</td>
                                        <td>{{ item.admin_fee | formatNumber }}</td>
                                        <td>{{ item.subAmount | formatNumber }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                                ...
                                <div class="col-md-11">
                                    <h5>
                                        <b v-show="!updateMeHeader"
                                            >CREATE INCENTIVE HEADER</b
                                        >
                                        <b v-show="updateMeHeader"
                                            >UPDATE INCENTIVE HEADER</b
                                        >
                                    </h5>
                                </div>
                                <div class="col-md-1">
        ...
        ></search-signatory>
        <search-incentivesHeader
            v-on:rowClick="incentivesHeaderClose($event)"
        ></search-incentivesHeader>
...
import searchIncentivesHeader from "../search/SearchAllowance/SearchIncentivesHeader.vue";
import searchEmployee from "../search/SearchAllowance/SearchEmployee.vue";
export default {
    components: {
        "search-signatory": searchSignatory,
        "search-incentivesHeader": searchIncentivesHeader,
    ...
    methods: {
        ...
        searchEmployeeButton() {
            if (!this.header.IHID) {
                return toast.fire({
                    icon: "warning",
                    title: "Please select Incentive Header to continue."
                });
            }
        ...
        employeeClose: function(row) {
            ...
            this.dtl.EmpID = row.EmpID;
            this.dtl.Chapa = row.Chapa;
            this.dtl.FName = row.FName;
            this.dtl.MName = row.MName;
            this.dtl.LName = row.LName;
            this.dtl.ExtName = row.ExtName;
        },
        openIncentivesHdr() {
            this.clearHeader("formHeader");
            $("#IncentivesHdrModal").modal("show");
        ...
        getDetail() {
            if (this.header.IHID) {
                axios
                    .get("api/incentives", {
                        params: { getDtl: true, id: this.header.IHID }
                    })
                    .then(response => {
        ...
        getTotal() {
            if (this.header.IHID) {
                axios
                    .get("api/incentives", {
                        params: { getTotal: true, id: this.header.IHID }
                    })
                    .then(response => {
                        if (response.data.length > 0) {
        ...
        getMarkup() {
            axios
                .get("api/markup")
                .then(response => {
                    this.markup = response.data[0].IncentiveMarkup;
        ...
        rowClick(row) {
            ...
            this.dtl.admin_fee = this.$root.formatNumberCommaRound(row.admin_fee);
        ...
        saveHdr() {
            ...
            this.$Progress.start();
            axios
                .post("api/incentivesHdr", data)
                .then(response => {
                    if (response.data.success) {
                        this.header = Object.assign({}, this.formHeader);
                        if (response.data.id) {
                            this.header.IHID = response.data.id;
                        }
                        toast.fire({
                            icon: "success",
        ...
        updateHdr() {
            if (!this.header.IHID) {
                return toast.fire({
                    icon: "warning",
                    title: "Please select/save INCENTIVES Header to continue."
                });
            }
        ...
        deleteHdr() {
            if (!this.header.IHID) {
                return toast.fire({
                    icon: "warning",
                    title: "Please select INCENTIVES SOA Header to continue."
                });
            }
            swal.fire({
                title: "Are you sure?",
                ...
                if (result.value) {
                    this.$Progress.start();
                    axios
                        .delete(`api/incentivesHdrDelete/${this.header.IHID}`)
                        .then(response => {
                            if (response.data.success) {
                                this.clearAll();
                                swal.fire(
        ...
        searchIncentivesHeader() {
            Fire.$emit("searchIncentivesHeader");
        },
        incentivesHeaderClose: function(row) {
        ...
        computeMarkup() {
            if (this.dtl.markup == 0) {
                this.dtl.admin_fee = "0.00";
            } else {
                if (this.dtl.markup) {
                    if (!isNaN(this.$root.formatNumber(this.dtl.Amount))) {
                        this.dtl.admin_fee = this.$root.formatNumberCommaRound(
                            (this.dtl.markup / 100) *
                                this.$root.formatNumber(this.dtl.Amount)
                        );
                    }
                }
            }
        ...
        computeAmount() {
            if (this.dtl.Amount) {
                if (!isNaN(this.$root.formatNumber(this.dtl.Amount))) {
                    this.dtl.subAmount = this.$root.formatNumberCommaRound(
                        this.$root.formatNumber(this.dtl.admin_fee) +
                            this.$root.formatNumber(this.dtl.Amount)
                    );
                } else {
                    this.dtl.subAmount = "0.00";
                }
            } else {
                this.dtl.subAmount = "0.00";
            }
        ...
        saveDtl() {
            ...
            data.admin_fee = this.$root.formatNumber(this.dtl.admin_fee);
            data.subAmount = this.$root.formatNumber(this.dtl.subAmount);
            data.hdr_idLink = this.header.IHID;
            this.$Progress.start();
            axios
                .post("api/incentivesDtl", data)
                .then(response => {
                    if (response.data.success) {
                        if (response.data.id) {
                            this.header.IEDID = response.data.id;
                        }
                        toast.fire({
                            icon: "success",
                            title: response.data.message
        ...
        deleteDtl() {
            if (!this.dtl.IEDID) {
                return toast.fire({
                    icon: "warning",
                    title: "Please select INCENTIVES Detail to continue."
                });
            }
            swal.fire({
                title: "Are you sure?",
                ...
                if (result.value) {
                    this.$Progress.start();
                    axios
                        .delete(`api/deleteIncentivesHDR/${this.dtl.IEDID}`)
                        .then(response => {
                            if (response.data.success) {
                                this.getDetail();
                                this.clearHeader("detail");
                ...
                .then(response => {
                    ...
                    if (response.data.success) {
                        this.reportData.url2 =
                            "api/reportIncentives?report=true&SOANo=" +
                            this.header.SOANo;
                    } else {
        ...
        postLedger() {
            swal.fire({
                ...
                if (result.value) {
                    ...
                    axios
                        .post("api/incentivesPostLedger", data)
                        .then(response => {
                            if (response.data.success) {
        ...
        viewReport() {
            if (this.modalPage == 1) {
                this.modalPage = 2;
                axios
                    .post("api/reportAllowanceSOASave", {
        ...
        reportModal() {
            ...
            this.reportData.body2 =
                "Please issue check in the name of GENERAL SERVICES MULTIPURPOSE COOPERATIVE.";
