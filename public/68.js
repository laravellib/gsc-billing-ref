(window["webpackJsonp"] = window["webpackJsonp"] || []).push([[68],{

/***/ "./node_modules/babel-loader/lib/index.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=script&lang=js&":
/*!*************************************************************************************************************************************************************************************!*\
  !*** ./node_modules/babel-loader/lib??ref--4-0!./node_modules/vue-loader/lib??vue-loader-options!./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=script&lang=js& ***!
  \*************************************************************************************************************************************************************************************/
/*! exports provided: default */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var util__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! util */ "./node_modules/util/util.js");
/* harmony import */ var util__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(util__WEBPACK_IMPORTED_MODULE_0__);
function _defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }

//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//

/* harmony default export */ __webpack_exports__["default"] = ({
  data: function data() {
    var _ref;

    return {
      signator: '',
      detailSOAIndividual: [],
      signatorSOA: '',
      content: '',
      arrayshit: [],
      csoas: [],
      csoacounts: [],
      csoatotals: [],
      soaforupdate: [],
      filteredblogs: [],
      checkedNames: [],
      clients: [],
      checkedNamesFilter: [],
      editmode: false,
      equalequal: false,
      jvls: [],
      search: '',
      jvcps: {},
      drivers: [],
      vehicles: {},
      rates: {},
      operators: {},
      batch: [],
      jvlfilter: [],
      jvlbalamt: [],
      var1: '',
      var2: '',
      first: '',
      second: '',
      eq: 'true',
      truevalue: '',
      falsevalue: '',
      allSelected: false,
      DateFrom: '',
      DateTo: '',
      SearchJeepPlateNo: '',
      JeepVehicleCollectionPayments: {},
      form: new Form((_ref = {
        JVLHDRID: '',
        JVLHDRIDFilter: '',
        JeepSoaDate: '',
        OVLNo: '',
        JeepIDLink: '',
        JeepPlateNo: '',
        DriverIDLink: '',
        DriverLastName: '',
        DriverFirstName: '',
        DriverMiddleName: '',
        DriverExtName: '',
        TruckerIDLink: '',
        TruckerLastName: '',
        TruckerFirstName: '',
        TruckerMiddleName: '',
        TruckerExtName: '',
        BillAmount: '',
        LessAdmin: '',
        LessFuel: '',
        NetTrucker: '',
        Status: '',
        SOANumber: '',
        SOADate: '',
        ChargeInvoiceNumber: '',
        GLCode: '',
        CostCenter: '',
        PerDayRate: '',
        NumberofDays: '',
        DriverName: '',
        TruckerName: '',
        CollectedAmount: '',
        CollectedAmountHDR: '',
        BalanceAmountHDR: '',
        BalanceAmount: '',
        ORCRNumber: '',
        ORCRDate: '',
        Remarks: '',
        JVLHDRID_Link: ''
      }, _defineProperty(_ref, "Status", ''), _defineProperty(_ref, "GSCSoaSeriesNo", ''), _defineProperty(_ref, "TotalAmount", ''), _defineProperty(_ref, "BilledName", ''), _defineProperty(_ref, "BilledToIDLink", ''), _defineProperty(_ref, "BilledAddress", ''), _defineProperty(_ref, "THRUName", ''), _defineProperty(_ref, "THRUPos", ''), _defineProperty(_ref, "NotedBy", ''), _defineProperty(_ref, "NotedByPos", ''), _defineProperty(_ref, "CheckedBy", ''), _defineProperty(_ref, "CheckedByPos", ''), _defineProperty(_ref, "PreparedBy", ''), _defineProperty(_ref, "PreparedByPos", ''), _defineProperty(_ref, "Soa_JeepID", ''), _defineProperty(_ref, "Salutation", ''), _defineProperty(_ref, "BodyofLetter", ''), _ref)),
      formSOA: new Form({
        Soa_JeepID: '',
        GSCSoaSeriesNo: '',
        Status: '',
        BilledToIDLink: '',
        BilledName: '',
        BilledAddress: '',
        THRUName: '',
        THRUPos: '',
        JeepSoaDate: '',
        PreparedBy: '',
        PreparedByPos: '',
        CheckedBy: '',
        CheckedByPos: '',
        NotedBy: '',
        NotedByPos: '',
        Salutation: '',
        BodyofLetter: '',
        TotalAmount: '',
        InvoiceNo: '',
        InvoiceDate: ''
      })
    };
  },
  mounted: function mounted() {// this.$parent.getSearchDriver();
    // this.$parent.getSearchOperator();
  },
  methods: {
    detailView: function detailView() {
      var _this = this;

      axios.get('/api/getcsoaforviewindiviual', {
        params: {
          Soa_JeepID: this.formSOA.Soa_JeepID
        }
      }).then(function (_ref2) {
        var data = _ref2.data;
        _this.detailSOAIndividual = data; // console.log(this.detailSOAIndividual);

        $('#viewdetailindividual').modal('show');
      })["catch"](function (err) {
        toast.fire({
          icon: 'error',
          title: 'Not Successfully fetched data.'
        });
      });
    },
    viewSOAForm: function viewSOAForm() {
      this.editmode = true;
      $('#addNewMultiple').modal('show');
    },
    check: function check(e) {
      var _this2 = this;

      this.form.JVLHDRID = this.checkedNames.join();
      axios.get('/api/getcsoasum', {
        params: {
          JVLHDRID: this.form.JVLHDRID
        }
      }).then(function (_ref3) {
        var data = _ref3.data;
        _this2.csoatotals = data;
        _this2.form.TotalAmount = _this2.csoatotals[0].BillAmount; // console.log(this.form.TotalAmount);
      })["catch"](function (err) {});
    },
    cancelSOA: function cancelSOA() {
      var _this3 = this;

      if (typeof this.formSOA.Soa_JeepID == 'undefined' || this.formSOA.Soa_JeepID == '') {
        swal.fire({
          icon: 'error',
          title: 'Oops...',
          text: 'No SOA Searched.'
        });
      } else {
        swal.fire({
          title: 'Are you sure?',
          text: "You won't be able to revert this!",
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          cancelButtonColor: '#d33',
          confirmButtonText: 'Yes!'
        }).then(function (result) {
          if (result.value) {
            _this3.$Progress.start(); //UPDATE SOA


            _this3.formSOA.Status = 'CANCELLED';
            axios.get('/api/cancelstatussoa', {
              params: {
                Soa_JeepID: _this3.formSOA.Soa_JeepID,
                Status: _this3.formSOA.Status
              }
            }).then(function (_ref4) {
              var data = _ref4.data;
              toast.fire({
                icon: 'success',
                title: 'Successfully Cancelled'
              });
            })["catch"](function (err) {
              toast.fire({
                icon: 'error',
                title: 'Not Successfull Cancelled.'
              });
            }); //END UPDATE SOA

            _this3.$Progress.finish();

            _this3.loadCSOA();

            $('#addNew').modal('hide');
            $('.modal-backdrop').remove();
          }
        });
      }
    },
    transmittalSOA: function transmittalSOA() {
      var _this4 = this;

      if (typeof this.formSOA.Soa_JeepID == 'undefined' || this.formSOA.Soa_JeepID == '') {
        swal.fire({
          icon: 'error',
          title: 'Oops...',
          text: 'No SOA Searched.'
        });
      } else {
        swal.fire({
          title: 'Are you sure?',
          text: "Do you want to Proceed?",
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          cancelButtonColor: '#d33',
          confirmButtonText: 'Yes!'
        }).then(function (result) {
          if (result.value) {
            _this4.$Progress.start(); //UPDATE SOA


            _this4.formSOA.Status = 'FOR TRANSMITTAL';
            axios.get('/api/cancelstatussoa', {
              params: {
                Soa_JeepID: _this4.formSOA.Soa_JeepID,
                Status: _this4.formSOA.Status
              }
            }).then(function (_ref5) {
              var data = _ref5.data;
              toast.fire({
                icon: 'success',
                title: 'Successfully Updated'
              });
            })["catch"](function (err) {
              toast.fire({
                icon: 'error',
                title: 'Not Successfully Updated'
              });
            }); //END UPDATE SOA

            _this4.$Progress.finish();

            _this4.loadCSOA();

            $('#addNew').modal('hide');
            $('.modal-backdrop').remove();
          }
        });
      }
    },
    individualSOA: function individualSOA(soa) {
      this.formSOA.reset();
      this.formSOA.Soa_JeepID = soa.Soa_JeepID;
      this.formSOA.GSCSoaSeriesNo = soa.GSCSoaSeriesNo;
      this.formSOA.Status = soa.Status;
      this.formSOA.BilledName = soa.BilledName;
      this.formSOA.BilledAddress = soa.BilledAddress;
      this.formSOA.THRUName = soa.THRUName;
      this.formSOA.THRUPos = soa.THRUPos;
      this.formSOA.JeepSoaDate = soa.JeepSoaDate;
      this.formSOA.PreparedBy = soa.PreparedBy;
      this.formSOA.PreparedByPos = soa.PreparedByPos;
      this.formSOA.CheckedBy = soa.CheckedBy;
      this.formSOA.CheckedByPos = soa.CheckedByPos;
      this.formSOA.NotedBy = soa.NotedBy;
      this.formSOA.NotedByPos = soa.NotedByPos;
      this.formSOA.Salutation = soa.Salutation;
      this.formSOA.BodyofLetter = soa.BodyofLetter;
      this.formSOA.TotalAmount = soa.TotalAmount;
      this.formSOA.InvoiceNo = soa.InvoiceNo;
      this.formSOA.InvoiceDate = soa.InvoiceDate;
      $('#addNew').modal('show');
    },
    soasetpaid: function soasetpaid() {
      var _this5 = this;

      this.formSOA.Soa_JeepID = this.checkedNames.join();

      if (typeof this.formSOA.Soa_JeepID == 'undefined' || this.formSOA.Soa_JeepID == '') {
        swal.fire({
          icon: 'error',
          title: 'Oops...',
          text: 'No SOA Searched.'
        });
      } else {
        swal.fire({
          title: 'Are you sure?',
          text: "You won't be able to revert this!",
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          cancelButtonColor: '#d33',
          confirmButtonText: 'Yes!'
        }).then(function (result) {
          if (result.value) {
            _this5.$Progress.start(); //UPDATE SOA


            _this5.formSOA.Status = 'PAID/COLLECTED';
            axios.get('/api/updatestatussoamultiple', {
              params: {
                Soa_JeepID: _this5.formSOA.Soa_JeepID,
                Status: _this5.formSOA.Status
              }
            }).then(function (_ref6) {
              var data = _ref6.data;

              _this5.$Progress.finish();

              toast.fire({
                icon: 'success',
                title: 'Successfully Paid/Collected'
              });
            })["catch"](function (err) {
              _this5.$Progress.finish();

              toast.fire({
                icon: 'error',
                title: 'Not successfully Paid/Collected'
              });
            }); //END UPDATE SOA

            _this5.loadCSOA();

            _this5.checkedNames = '';
            _this5.allSelected = false; //location.reload();
          }
        });
      }
    },
    soapost: function soapost() {
      var _this6 = this;

      if (typeof this.formSOA.Soa_JeepID == 'undefined' || this.formSOA.Soa_JeepID == '') {
        swal.fire({
          icon: 'error',
          title: 'Oops...',
          text: 'No SOA Searched.'
        });
      } else {
        swal.fire({
          title: 'Are you sure?',
          text: "You won't be able to revert this!",
          icon: 'warning',
          showCancelButton: true,
          confirmButtonColor: '#3085d6',
          cancelButtonColor: '#d33',
          confirmButtonText: 'Yes!'
        }).then(function (result) {
          if (result.value) {
            _this6.$Progress.start();

            $('#addNew').modal('hide');
            _this6.formSOA.Status = 'POSTED';
            axios.get('/api/updatestatussoa', {
              params: {
                Soa_JeepID: _this6.formSOA.Soa_JeepID,
                Status: _this6.formSOA.Status
              }
            }).then(function (_ref7) {//this.csoas = data;

              var data = _ref7.data;
            })["catch"](function (err) {}); //UPDATE SOA

            _this6.form.JVLHDRID = _this6.checkedNames.join();
            axios.get('/api/updatejvlsoa', {
              params: {
                JVLHDRID: _this6.form.JVLHDRID,
                SOA_JeepID: _this6.formSOA.Soa_JeepID,
                GSCSoaSeriesNo: _this6.formSOA.GSCSoaSeriesNo,
                JeepSoaDate: _this6.formSOA.JeepSoaDate
              }
            }).then(function (_ref8) {
              var data = _ref8.data;
              _this6.csoatotals = data;
            })["catch"](function (err) {}); //END UPDATE SOA

            swal.fire('Posted!', 'Your file has been posted. The page will refresh after.', 'success');

            _this6.$Progress.finish();

            location.reload();
          }
        });
      }
    },
    fakeVmodel: function fakeVmodel(e) {
      this.content = e.target.innerText;
    },
    downloadPDFSOA: function downloadPDFSOA() {
      if (typeof this.formSOA.Soa_JeepID == 'undefined' || this.formSOA.Soa_JeepID == '') {
        swal.fire({
          icon: 'error',
          title: 'Oops...',
          text: 'No SOA Searched.'
        });
      } else {
        // var cont  = "<div align='center' text-indent='50px' style='font-size: 14px;'>  &#160;"+document.getElementById('stringinput').innerHTML +"</div>";
        //console.log(cont);
        var imageData = 'data:image/png;base64,';
        window.jsPDF = __webpack_require__(/*! jspdf */ "./node_modules/jspdf/dist/jspdf.min.js");
        var pdf = new jsPDF();
        pdf.setProperties({
          title: 'SOA of ' + this.formSOA.GSCSoaSeriesNo + '.pdf'
        }); //HEADER

        pdf.setFont('helvetica');
        pdf.setFontType('bold');
        pdf.setFontSize(14);
        pdf.text('GENERAL SERVICES MULTI-PURPOSE COOPERATIVE', 105, 10, 'center');
        pdf.setFontSize(10);
        pdf.setFontType('regular');
        pdf.text('Office Address: Borja Road, Damilag, Manolo Fortich, Bukidnon', 105, 14, 'center');
        pdf.text('CDA # 9520-10019987-1 / TIN 411-478-949-000', 105, 18, 'center');
        pdf.addImage(imageData, 'JPEG', 20, 5, 20, 20);
        pdf.setLineWidth(0.5);
        pdf.line(15, 27, 195, 27);
        pdf.setLineWidth(1);
        pdf.line(15, 28, 195, 28); //END HEADER
        //BODY

        pdf.setFont('helvetica');
        pdf.setFontSize(14);
        pdf.setFontType('bold');
        pdf.text('STATEMENT OF ACCOUNT', 105, 40, 'center');
        pdf.setFont('helvetica');
        pdf.setFontSize(10);
        pdf.setFontType('regular');
        pdf.text('SOA DATE:', 30, 50, 'left');
        pdf.setFontType('bold');
        pdf.setFontSize(9);
        pdf.text(this.formSOA.JeepSoaDate, 50, 50, 'left'); //ACCROSS

        pdf.setFontType('regular');
        pdf.setFontSize(10);
        pdf.text('SOA #:', 140, 50, 'left');
        pdf.setFontType('bold');
        pdf.setFontSize(9);
        pdf.text(this.formSOA.GSCSoaSeriesNo, 152, 50, 'left'); //SECOND LINE

        pdf.setFont('helvetica');
        pdf.setFontSize(10);
        pdf.setFontType('regular');
        pdf.text('BILLED TO:', 30, 58, 'left');
        pdf.setFontType('bold');
        pdf.setFontSize(9);
        pdf.text(this.formSOA.BilledName, 50, 58, 'left'); //ACCROSS

        pdf.setFontType('regular');
        pdf.setFontSize(10); // pdf.text('CHARGE INVOICE #:',117, 58,'left');

        pdf.text('CI #:', 144, 58, 'left');
        pdf.setFontType('bold');
        pdf.setFontSize(9);
        pdf.text(this.formSOA.InvoiceNo, 152, 58, 'left'); // console.log(this.formSOA);
        //END

        pdf.setFontType('regular');
        pdf.setFontSize(10);
        pdf.text('THRU:', 30, 66, 'left');
        pdf.setFontType('bold');
        pdf.setFontSize(9);
        pdf.text(this.formSOA.THRUName, 50, 66, 'left');
        pdf.setFontType('regular');
        pdf.setFontSize(10);
        pdf.text(this.formSOA.THRUPos, 50, 69, 'left');
        pdf.text('CI Date:', 139, 66, 'left');
        pdf.setFontType('bold');
        pdf.setFontSize(9);
        pdf.text(this.formSOA.InvoiceDate, 152, 66, 'left'); //END BODY
        //SALUTATION

        pdf.setFontType('regular');
        pdf.setFontSize(10);
        pdf.text(this.formSOA.Salutation, 30, 78, 'left'); //END SALUTATION
        //BODY OF LETTER
        //pdf.text('            ' +this.content,30, 80,{maxWidth: 145, align: "justify"});

        pdf.fromHTML(this.formSOA.BodyofLetter, 30, 80, {
          width: 145
        });
        pdf.setFontType('regular');
        pdf.setFontSize(12);

        var numeral = __webpack_require__(/*! numeral */ "./node_modules/numeral/numeral.js");

        var stringSOATotalAmount = numeral(this.formSOA.TotalAmount).format('0,0.00');
        var textWidth = pdf.getTextWidth('Php ' + stringSOATotalAmount);
        console.log(textWidth);
        pdf.text('Php ' + stringSOATotalAmount, 105, 105, 'center');
        pdf.setLineWidth(0.2);
        pdf.line(94.5, 106, 94.5 + textWidth, 106);
        pdf.setFontType('regular');
        pdf.setFontSize(10);
        pdf.text('Please issue check in the name of', 30, 117, 'left');
        pdf.setFontType('bold');
        pdf.setFontSize(9);
        pdf.text('GENERAL SERVICES MULTIPURPOSE COOPERATIVE.', 78.5, 117, 'left');
        pdf.setFontType('regular');
        pdf.setFontSize(10);
        pdf.text('Prepared by:', 30, 130, 'left');
        pdf.text(this.formSOA.PreparedBy, 30, 140, 'left');
        var textWidths = pdf.getTextWidth(this.formSOA.PreparedBy);
        pdf.setLineWidth(0.2);
        pdf.line(30, 141, 30 + textWidths, 141);
        pdf.text(this.formSOA.PreparedByPos, 30, 144, 'left');
        pdf.setFontType('regular');
        pdf.setFontSize(10);
        pdf.text('Checked by:', 30, 160, 'left');
        pdf.text(this.formSOA.CheckedBy, 30, 170, 'left');
        var textWidthss = pdf.getTextWidth(this.formSOA.CheckedBy);
        pdf.setLineWidth(0.2);
        pdf.line(30, 171, 30 + textWidthss, 171);
        pdf.text(this.formSOA.CheckedByPos, 30, 174, 'left');
        pdf.setFontType('regular');
        pdf.setFontSize(10);
        pdf.text('Noted by:', 30, 190, 'left');
        pdf.text(this.formSOA.NotedBy, 30, 200, 'left');
        var textWidthsss = pdf.getTextWidth(this.formSOA.NotedBy);
        pdf.setLineWidth(0.2);
        pdf.line(30, 201, 30 + textWidthsss, 201);
        pdf.text(this.formSOA.NotedByPos, 30, 204, 'left'); //END BODDY OF LETTER
        //FOOTER

        pdf.setLineWidth(1);
        pdf.line(15, 250, 195, 250);
        pdf.setFontType('regular');
        pdf.setFontSize(10);
        pdf.text('Contact #s: 0917-7011814 / 09017-7022814 / 0917-3199733 / 0917-77796580', 105, 255, 'center');
        pdf.setFontType('bold');
        pdf.setFontSize(9);
        pdf.text('Website: www.gsc.com.ph / Email add: gensercoop@yahoo.com', 105, 259, 'center');
        pdf.setFontType('bold');
        pdf.setFontSize(14);
        pdf.setFontType('italic');
        pdf.text('"Resourceful Workmanship"', 105, 264, 'center'); //END FOOTER
        //pdf.save('info.pdf');

        window.open(pdf.output('bloburl'), '_blank');
      }
    },
    downloadPDF: function downloadPDF() {
      // var cont  = "<div align='center' text-indent='50px' style='font-size: 14px;'>  &#160;"+document.getElementById('stringinput').innerHTML +"</div>";
      //console.log(cont);
      var imageData = 'data:image/png;base64,';
      var pdf = new jsPDF();
      pdf.setProperties({
        title: 'SOA of ' + this.formSOA.GSCSoaSeriesNo + '.pdf'
      }); //HEADER

      pdf.setFont('helvetica');
      pdf.setFontType('bold');
      pdf.setFontSize(14);
      pdf.text('GENERAL SERVICES MULTI-PURPOSE COOPERATIVE', 105, 10, 'center');
      pdf.setFontSize(10);
      pdf.setFontType('regular');
      pdf.text('Office Address: Borja Road, Damilag, Manolo Fortich, Bukidnon', 105, 14, 'center');
      pdf.text('CDA # 9520-10019987-1 / TIN 411-478-949-000', 105, 18, 'center');
      pdf.addImage(imageData, 'JPEG', 20, 5, 20, 20);
      pdf.setLineWidth(0.5);
      pdf.line(15, 27, 195, 27);
      pdf.setLineWidth(1);
      pdf.line(15, 28, 195, 28); //END HEADER
      //BODY

      pdf.setFont('helvetica');
      pdf.setFontSize(14);
      pdf.setFontType('bold');
      pdf.text('STATEMENT OF ACCOUNT', 105, 40, 'center');
      pdf.setFont('helvetica');
      pdf.setFontSize(10);
      pdf.setFontType('regular');
      pdf.text('BILLED TO:', 30, 50, 'left');
      pdf.setFontType('bold');
      pdf.setFontSize(9);
      pdf.text(this.form.BilledName, 50, 50, 'left'); //ACCROSS

      pdf.setFontType('regular');
      pdf.setFontSize(10);
      pdf.text('SOA #:', 140, 50, 'left');
      pdf.setFontType('bold');
      pdf.setFontSize(9);
      pdf.text(this.form.GSCSoaSeriesNo, 152, 50, 'left'); //END

      pdf.setFontType('regular');
      pdf.setFontSize(10);
      pdf.text('THRU:', 30, 58, 'left');
      pdf.setFontType('bold');
      pdf.setFontSize(9);
      pdf.text(this.form.THRUName, 50, 58, 'left');
      pdf.setFontType('regular');
      pdf.setFontSize(10);
      pdf.text(this.form.THRUPos, 50, 61, 'left'); //END BODY
      //SALUTATION

      pdf.setFontType('regular');
      pdf.setFontSize(10);
      pdf.text(this.form.Salutation, 30, 70, 'left'); //END SALUTATION
      //BODY OF LETTER
      //pdf.text('            ' +this.content,30, 80,{maxWidth: 145, align: "justify"});

      pdf.fromHTML(this.form.BodyofLetter, 30, 80, {
        width: 145
      });
      pdf.setFontType('regular');
      pdf.setFontSize(12);

      var numeral = __webpack_require__(/*! numeral */ "./node_modules/numeral/numeral.js");

      var stringTotalAmount = numeral(this.form.TotalAmount).format('0,0.00');
      var textWidth = pdf.getTextWidth('Php ' + stringTotalAmount);
      console.log(textWidth);
      pdf.text('Php ' + stringTotalAmount, 105, 105, 'center');
      pdf.setLineWidth(0.2);
      pdf.line(94.5, 106, 94.5 + textWidth, 106);
      pdf.setFontType('regular');
      pdf.setFontSize(10);
      pdf.text('Please issue check in the name of', 30, 117, 'left');
      pdf.setFontType('bold');
      pdf.setFontSize(9);
      pdf.text('GENERAL SERVICES MULTIPURPOSE COOPERATIVE.', 78.5, 117, 'left');
      pdf.setFontType('regular');
      pdf.setFontSize(10);
      pdf.text('Prepared by:', 30, 130, 'left');
      pdf.text(this.form.PreparedBy, 30, 140, 'left');
      var textWidths = pdf.getTextWidth(this.form.PreparedBy);
      pdf.setLineWidth(0.2);
      pdf.line(30, 141, 30 + textWidths, 141);
      pdf.text(this.form.PreparedByPos, 30, 144, 'left');
      pdf.setFontType('regular');
      pdf.setFontSize(10);
      pdf.text('Checked by:', 30, 160, 'left');
      pdf.text(this.form.CheckedBy, 30, 170, 'left');
      var textWidthss = pdf.getTextWidth(this.form.CheckedBy);
      pdf.setLineWidth(0.2);
      pdf.line(30, 171, 30 + textWidthss, 171);
      pdf.text(this.form.CheckedByPos, 30, 174, 'left');
      pdf.setFontType('regular');
      pdf.setFontSize(10);
      pdf.text('Noted by:', 30, 190, 'left');
      pdf.text(this.form.NotedBy, 30, 200, 'left');
      var textWidthsss = pdf.getTextWidth(this.form.NotedBy);
      pdf.setLineWidth(0.2);
      pdf.line(30, 201, 30 + textWidthsss, 201);
      pdf.text(this.form.NotedByPos, 30, 204, 'left'); //END BODDY OF LETTER
      //FOOTER

      pdf.setLineWidth(1);
      pdf.line(15, 250, 195, 250);
      pdf.setFontType('regular');
      pdf.setFontSize(10);
      pdf.text('Contact #s: 0917-7011814 / 09017-7022814 / 0917-3199733 / 0917-77796580', 105, 255, 'center');
      pdf.setFontType('bold');
      pdf.setFontSize(9);
      pdf.text('Website: www.gsc.com.ph / Email add: gensercoop@yahoo.com', 105, 259, 'center');
      pdf.setFontType('bold');
      pdf.setFontSize(14);
      pdf.setFontType('italic');
      pdf.text('"Resourceful Workmanship"', 105, 264, 'center'); //END FOOTER
      //pdf.save('info.pdf');

      window.open(pdf.output('bloburl'), '_blank');
    },
    selectAll: function selectAll() {
      this.checkedNames = [];

      if (this.allSelected == false) {
        for (var user in this.filteredBlogs) {
          this.checkedNames.push(this.filteredBlogs[user].Soa_JeepID);
        }
      }
    },
    select: function select() {
      this.allSelected = false;
    },
    updateTitleSOA: function updateTitleSOA(updatedTitleSOA) {
      this.formSOA.Soa_JeepID = updatedTitleSOA.Soa_JeepID;
      this.formSOA.GSCSoaSeriesNo = updatedTitleSOA.GSCSoaSeriesNo;
      this.formSOA.JeepSoaDate = updatedTitleSOA.JeepSoaDate;
      this.formSOA.Status = updatedTitleSOA.Status;
      this.formSOA.BilledToIDLink = updatedTitleSOA.BilledToIDLink;
      this.formSOA.BilledName = updatedTitleSOA.BilledName;
      this.formSOA.BilledAddress = updatedTitleSOA.BilledAddress;
      this.formSOA.THRUName = updatedTitleSOA.THRUName;
      this.formSOA.THRUPos = updatedTitleSOA.THRUPos;
      this.formSOA.PreparedBy = updatedTitleSOA.PreparedBy;
      this.formSOA.PreparedByPos = updatedTitleSOA.PreparedByPos;
      this.formSOA.CheckedBy = updatedTitleSOA.CheckedBy;
      this.formSOA.CheckedByPos = updatedTitleSOA.CheckedByPos;
      this.formSOA.NotedBy = updatedTitleSOA.NotedBy;
      this.formSOA.NotedByPos = updatedTitleSOA.NotedByPos;
      this.formSOA.Salutation = updatedTitleSOA.Salutation;
      this.formSOA.BodyofLetter = updatedTitleSOA.BodyofLetter;
      this.formSOA.InvoiceNo = updatedTitleSOA.InvoiceNo;
      this.formSOA.InvoiceDate = updatedTitleSOA.InvoiceDate;
      this.formSOA.TotalAmount = parseFloat(updatedTitleSOA.TotalAmount).toFixed(2); // console.log(updatedTitle);
    },
    getResults: function getResults() {
      var _this7 = this;

      var page = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : 1;
      axios.get('api/jvl?page=' + page).then(function (response) {
        _this7.jeepvehiclelog = response.data;
      });
    },
    newModal: function newModal() {
      var _this8 = this;

      this.form.JVLHDRID = this.checkedNames.join();

      if (this.form.JVLHDRID == '') {
        swal.fire({
          icon: 'error',
          title: 'Oops...',
          text: 'No Transaction Selected.'
        });
      } else {
        axios.get('/api/getcsoasum', {
          params: {
            JVLHDRID: this.form.JVLHDRID
          }
        }).then(function (_ref9) {
          var data = _ref9.data;

          if (_this8.checkedNames == '') {
            _this8.$Progress.fail();

            swal.fire({
              icon: 'error',
              title: 'Oops...',
              text: 'No Transaction Selected.'
            });
          } else {
            _this8.editmode = false;

            _this8.form.reset();

            _this8.form.Status = 'ACTIVE'; //this.form.TotalAmount=parseFloat(this.total).toFixed(2);

            _this8.form.TotalAmount = '';
            _this8.form.JVLHDRID = _this8.checkedNames.join();
            axios.get('/api/getcsoasum', {
              params: {
                JVLHDRID: _this8.form.JVLHDRID
              }
            }).then(function (_ref10) {
              var data = _ref10.data;
              _this8.csoatotals = data;
              _this8.form.TotalAmount = parseFloat(_this8.csoatotals[0].BillAmount).toFixed(2);
              console.log(_this8.form.TotalAmount);
            })["catch"](function (err) {});
            var d = new Date();
            var today = new Date().toISOString().slice(0, 10);
            _this8.form.JeepSoaDate = today;
            axios.get('/api/getcsoa', {
              params: {
                JeepSoaDate: _this8.form.JeepSoaDate
              }
            }).then(function (_ref11) {
              var data = _ref11.data;
              _this8.csoacounts = data;

              if (_this8.csoacounts[0].SOAIncrement == 0) {
                _this8.form.GSCSoaSeriesNo = 'PUJ' + new Date().getFullYear() + new Date().toISOString().slice(5, 7) + '-1';
              } else {
                var num = _this8.csoacounts[0].SOAIncrement + 1;
                _this8.form.GSCSoaSeriesNo = 'PUJ' + new Date().getFullYear() + new Date().toISOString().slice(5, 7) + '-' + num;
              }
            })["catch"](function (err) {});
            $('#addNew').modal('show');
          }
        })["catch"](function (err) {});
      }
    },
    soaForm: function soaForm() {
      this.form.JVLHDRID = this.checkedNames.join();

      if (this.form.JVLHDRID == '') {
        swal.fire({
          icon: 'error',
          title: 'Oops...',
          text: 'No Transaction Selected.'
        });
      } else {
        this.formSOA.reset();
        $('#addNewMultiple').modal('show');
      }
    },
    doSomething: function doSomething(data) {
      console.log(data);
    },
    loadJVL: function loadJVL() {
      var _this9 = this;

      //axios.get("api/jvl").then(({ data }) => (this.jvls = data));
      axios.get('api/getjvl').then(function (response) {
        _this9.jvls = response.data; // console.log(this.jvls);
      })["catch"](function (error) {
        console.log(error);
      });
    },
    loadCSOA: function loadCSOA() {
      var _this10 = this;

      //axios.get("api/jvl").then(({ data }) => (this.jvls = data));
      axios.get('api/getcsoasoa').then(function (response) {
        _this10.csoas = response.data;
        console.log(_this10.csoas);
      })["catch"](function (error) {
        console.log(error);
      });
    },
    loadJVCP: function loadJVCP() {
      var _this11 = this;

      axios.get('api/jvcp').then(function (_ref12) {
        var data = _ref12.data;
        return _this11.jvcps = data;
      });
    },
    createJSOA: function createJSOA() {
      var _this12 = this;

      this.form.BodyofLetter = "<p style='font-size:12px;text-justify: inter-word;'>" + document.getElementById('stringinput').innerHTML + '</p>';
      this.$Progress.start();
      this.form.post('api/csoa').then(function () {
        var today = new Date().toISOString().slice(0, 10);

        _this12.loadJVL();

        _this12.loadCSOA();
      })["catch"](function () {
        _this12.$Progress.fail();

        toast.fire({
          icon: 'error',
          title: 'Jeep SOA Form not added successfully'
        });
      });
      this.$Progress.finish();
      toast.fire({
        icon: 'success',
        title: 'SOA successfully added. The page will refresh in a second.'
      });
      $('#addNew').modal('hide');
      location.reload();
    },
    updateJSOA: function updateJSOA() {
      var _this13 = this;

      this.form.BodyofLetter = "<p style='font-size:12px;text-justify: inter-word;'>" + document.getElementById('stringinput').innerHTML + '</p>';
      this.$Progress.start();
      this.formSOA.post('api/updatejsoa').then(function () {
        toast.fire({
          icon: 'success',
          title: 'Jeep SOA Form successfully updated'
        });
      })["catch"](function () {
        _this13.$Progress.fail();

        toast.fire({
          icon: 'error',
          title: 'Jeep SOA Form not added successfully'
        });
      });
      this.$Progress.finish();
    },
    createJVCP: function createJVCP() {
      var _this14 = this;

      this.form.post('api/jvcp').then(function () {
        toast.fire({
          icon: 'success',
          title: 'Jeep Vehicle Log successfully created'
        });

        _this14.getJVCP();

        _this14.form.CollectedAmount = ''; //this.form.BalanceAmount=0;

        _this14.form.ORCRNumber = '';
        _this14.form.ORCRDate = '';
        _this14.form.Remarks = '';

        _this14.getJVLBalanceAmount();
      })["catch"](function () {
        _this14.$Progress.fail();

        toast.fire({
          icon: 'error',
          title: 'Jeep Vehicle Collection Payment not added successfully'
        });
      });
    },
    getJVLBalanceAmount: function getJVLBalanceAmount() {
      var _this15 = this;

      axios.get('/api/getjvlbalanceamount', {
        params: {
          JVLHDRID: this.form.JVLHDRID
        }
      }).then(function (response) {
        _this15.jvlbalamt = response.data;
        _this15.form.BalanceAmountHDR = _this15.jvlbalamt[0].BalanceAmount; // console.log(this.jvlbalamt[0].BalanceAmount);
      })["catch"](function (err) {});
    },
    createJVCPMultiple: function createJVCPMultiple() {
      var _this16 = this;

      if (this.form.BalanceAmount == 0) {
        this.form.Status = 'PAID';
      } else {
        this.form.Status = 'POSTED';
      }

      this.editmode = false;
      swal.fire({
        title: 'Are you sure you watn to save?',
        text: "You won't be able to revert this!",
        icon: 'warning',
        showCancelButton: true,
        confirmButtonColor: '#3085d6',
        cancelButtonColor: '#d33',
        confirmButtonText: 'Yes, save it!'
      }).then(function (result) {
        if (result.value) {
          _this16.$Progress.start(); ////console.log(this.checkedNames);


          var arr = 0;

          for (var i = 0; i < _this16.checkedNames.length; i++) {
            arr = i; // console.log(this.checkedNames.length);

            _this16.form.JVLHDRID_Link = _this16.checkedNames[i]; //console.log(this.form.JVLHDRID_Link);
            //UPDATE VEHICLE LOG HDR

            _this16.form.put('api/jvljvcp/' + _this16.checkedNames[i]);

            $('#addNewMultiple').modal('hide'); //END UPDATE
            //CREATE INSERT TO JEEP VEHICLE COLLECTION PAYMENT

            _this16.form.post('api/jvcp').then(function () {
              //toast.fire({
              //   icon: "success",
              //  title: "Transaction done."
              //});
              swal.fire({
                position: 'center',
                icon: 'success',
                title: 'Your work has been saved',
                showConfirmButton: false,
                timer: 1500
              });
            }); //END CREATE

          }

          _this16.$Progress.finish(); //location.reload();


          _this16.loadJVL();

          _this16.checkedNames = [];
        }
      });
    }
  },
  created: function created() {
    var _this17 = this;

    Fire.$on('searchingJVL', function () {
      var query = _this17.$parent.search;
      axios.get('api/findJVL?q=' + query).then(function (data) {
        _this17.jvls = data.data;
      })["catch"](function () {});
    });
    this.loadCSOA();
    var today = new Date().toISOString().slice(0, 10);
    this.DateFrom = today;
    this.DateTo = today;
    this.form.TotalAmount = '0.00'; //setInterval(() => this.loadDriver(),3000);
  },
  computed: {
    filteredBlogs: function filteredBlogs() {
      var vm = this;
      var startdate = vm.DateFrom;
      var enddate = vm.DateTo;
      return _.filter(vm.csoas, function (data) {
        if (_.isNull(startdate) && _.isNull(enddate)) {
          return true;
          this.filteredblogs = data.JeepSoaDate;
        } else {
          var date = data.JeepSoaDate;
          return date >= startdate && date <= enddate;
        }
      });
      /*
                 else{
                   this.form.DateFrom = "";
                   this.form.DateTo = ""
                   //return this.jvls.filter(jvl =>{
                   //return jvl.JeepPlateNo.includes(this.form.JeepPlateNo)
                 //});
                 return this.jvls.filter((samsung) => {
                   return this.SearchJeepPlateNo.toLowerCase().split(' ').every(v => samsung.JeepPlateNo.toLowerCase().includes(v));
                   this.filteredblogs = samsung.JVLHDRID;
               });
                 }*/
    },
    total: function total() {
      var vm = this;
      /*vm.sum = vm.filteredBlogs.filter((item) =>item.BillAmount)
                               .map((item) => +item.BillAmount)
                               .reduce((sum, current) => sum + current);
                           console.log(vm.sum);
                        return vm.sum;*/

      return vm.filteredBlogs.reduce(function (sum, item) {
        var tot = sum + item.BillAmount;
        vm.sum = tot;
        return sum + item.BillAmount;
      }, 0);
    }
  }
});

/***/ }),

/***/ "./node_modules/css-loader/index.js?!./node_modules/vue-loader/lib/loaders/stylePostLoader.js!./node_modules/postcss-loader/src/index.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=style&index=0&lang=css&":
/*!********************************************************************************************************************************************************************************************************************************************************************************************!*\
  !*** ./node_modules/css-loader??ref--6-1!./node_modules/vue-loader/lib/loaders/stylePostLoader.js!./node_modules/postcss-loader/src??ref--6-2!./node_modules/vue-loader/lib??vue-loader-options!./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=style&index=0&lang=css& ***!
  \********************************************************************************************************************************************************************************************************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

exports = module.exports = __webpack_require__(/*! ../../../../node_modules/css-loader/lib/css-base.js */ "./node_modules/css-loader/lib/css-base.js")(false);
// imports


// module
exports.push([module.i, "\ninput[data-readonly] {\r\n\tpointer-events: none;\n}\n#stringinput {\r\n\tmin-width: 500px;\r\n\tmin-height: 80px;\r\n\tmax-width: 500px;\r\n\tmax-height: 80px;\r\n\ttext-align: left;\r\n\toverflow-y: auto;\n}\r\n", ""]);

// exports


/***/ }),

/***/ "./node_modules/style-loader/index.js!./node_modules/css-loader/index.js?!./node_modules/vue-loader/lib/loaders/stylePostLoader.js!./node_modules/postcss-loader/src/index.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=style&index=0&lang=css&":
/*!************************************************************************************************************************************************************************************************************************************************************************************************************************!*\
  !*** ./node_modules/style-loader!./node_modules/css-loader??ref--6-1!./node_modules/vue-loader/lib/loaders/stylePostLoader.js!./node_modules/postcss-loader/src??ref--6-2!./node_modules/vue-loader/lib??vue-loader-options!./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=style&index=0&lang=css& ***!
  \************************************************************************************************************************************************************************************************************************************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {


var content = __webpack_require__(/*! !../../../../node_modules/css-loader??ref--6-1!../../../../node_modules/vue-loader/lib/loaders/stylePostLoader.js!../../../../node_modules/postcss-loader/src??ref--6-2!../../../../node_modules/vue-loader/lib??vue-loader-options!./JeepSOAList.vue?vue&type=style&index=0&lang=css& */ "./node_modules/css-loader/index.js?!./node_modules/vue-loader/lib/loaders/stylePostLoader.js!./node_modules/postcss-loader/src/index.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=style&index=0&lang=css&");

if(typeof content === 'string') content = [[module.i, content, '']];

var transform;
var insertInto;



var options = {"hmr":true}

options.transform = transform
options.insertInto = undefined;

var update = __webpack_require__(/*! ../../../../node_modules/style-loader/lib/addStyles.js */ "./node_modules/style-loader/lib/addStyles.js")(content, options);

if(content.locals) module.exports = content.locals;

if(false) {}

/***/ }),

/***/ "./node_modules/vue-loader/lib/loaders/templateLoader.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=template&id=67b85895&":
/*!*****************************************************************************************************************************************************************************************************************************!*\
  !*** ./node_modules/vue-loader/lib/loaders/templateLoader.js??vue-loader-options!./node_modules/vue-loader/lib??vue-loader-options!./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=template&id=67b85895& ***!
  \*****************************************************************************************************************************************************************************************************************************/
/*! exports provided: render, staticRenderFns */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "render", function() { return render; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "staticRenderFns", function() { return staticRenderFns; });
var render = function() {
  var _vm = this
  var _h = _vm.$createElement
  var _c = _vm._self._c || _h
  return _c("div", { staticClass: "container", attrs: { id: "sweget" } }, [
    _c(
      "nav",
      {
        staticClass:
          "navbar navbar-dark bg-dark navbar-expand-lg navbar-light bg-light"
      },
      [
        _c(
          "div",
          {
            staticClass: "collapse navbar-collapse",
            attrs: { id: "navbarNavDropdown" }
          },
          [
            _c("ul", { staticClass: "navbar-nav" }, [
              _c("li", { staticClass: "nav-item dropdown" }, [
                _c(
                  "a",
                  {
                    staticClass: "nav-link dropdown-toggle",
                    attrs: {
                      href: "#",
                      id: "navbarDropdownMenuLink",
                      role: "button",
                      "data-toggle": "dropdown",
                      "aria-haspopup": "true",
                      "aria-expanded": "false",
                      "data-target": "#mlist"
                    }
                  },
                  [
                    _vm._v(
                      "\n                                Master File\n                                "
                    )
                  ]
                ),
                _vm._v(" "),
                _c(
                  "ul",
                  {
                    staticClass: "dropdown-menu",
                    attrs: {
                      "aria-labelledby": "navbarDropdownMenuLink",
                      id: "mlist"
                    }
                  },
                  [
                    _c(
                      "li",
                      [
                        _c(
                          "router-link",
                          {
                            staticClass: "dropdown-item",
                            attrs: { to: "/driverlist" }
                          },
                          [_c("a", [_vm._v("Driver List")])]
                        )
                      ],
                      1
                    ),
                    _vm._v(" "),
                    _c(
                      "li",
                      [
                        _c(
                          "router-link",
                          {
                            staticClass: "dropdown-item",
                            attrs: { to: "/operatorlist" }
                          },
                          [_c("a", [_vm._v("Operator List")])]
                        )
                      ],
                      1
                    ),
                    _vm._v(" "),
                    _c(
                      "li",
                      [
                        _c(
                          "router-link",
                          {
                            staticClass: "dropdown-item",
                            attrs: { to: "/vehiclelist" }
                          },
                          [_c("a", [_vm._v("JEEP Vehicle List")])]
                        )
                      ],
                      1
                    ),
                    _vm._v(" "),
                    _c(
                      "li",
                      [
                        _c(
                          "router-link",
                          {
                            staticClass: "dropdown-item",
                            attrs: { to: "/vehicletype" }
                          },
                          [_c("a", [_vm._v("Vehicle Type List")])]
                        )
                      ],
                      1
                    ),
                    _vm._v(" "),
                    _c(
                      "li",
                      [
                        _c(
                          "router-link",
                          {
                            staticClass: "dropdown-item",
                            attrs: { to: "/vehiclerate" }
                          },
                          [_c("a", [_vm._v("JEEP Rate List")])]
                        )
                      ],
                      1
                    ),
                    _vm._v(" "),
                    _c(
                      "li",
                      [
                        _c(
                          "router-link",
                          {
                            staticClass: "dropdown-item",
                            attrs: { to: "/clientlist" }
                          },
                          [_c("a", [_vm._v("Client List")])]
                        )
                      ],
                      1
                    ),
                    _vm._v(" "),
                    _c(
                      "li",
                      [
                        _c(
                          "router-link",
                          {
                            staticClass: "dropdown-item",
                            attrs: { to: "/ratePercent" }
                          },
                          [_c("a", [_vm._v("Admin Rate Percentage")])]
                        )
                      ],
                      1
                    )
                  ]
                )
              ]),
              _vm._v(" "),
              _c("li", { staticClass: "nav-item dropdown" }, [
                _c(
                  "a",
                  {
                    staticClass: "nav-link dropdown-toggle",
                    attrs: {
                      href: "#",
                      id: "navbarDropdownMenuLink",
                      role: "button",
                      "data-toggle": "dropdown",
                      "aria-haspopup": "true",
                      "aria-expanded": "false",
                      "data-target": "#trans"
                    }
                  },
                  [
                    _vm._v(
                      "\n                                Transactions\n                                "
                    )
                  ]
                ),
                _vm._v(" "),
                _c(
                  "ul",
                  {
                    staticClass: "dropdown-menu",
                    attrs: {
                      "aria-labelledby": "navbarDropdownMenuLink",
                      id: "trans"
                    }
                  },
                  [
                    _c(
                      "li",
                      [
                        _c(
                          "router-link",
                          {
                            staticClass: "dropdown-item",
                            attrs: { to: "/jeepvehiclelogentry" }
                          },
                          [_c("a", [_vm._v("Jeep Vehicle Log Entry")])]
                        )
                      ],
                      1
                    ),
                    _vm._v(" "),
                    _c(
                      "li",
                      [
                        _c(
                          "router-link",
                          {
                            staticClass: "dropdown-item",
                            attrs: { to: "/createsoa" }
                          },
                          [_c("a", [_vm._v("Create Jeep SOA")])]
                        )
                      ],
                      1
                    ),
                    _vm._v(" "),
                    _c(
                      "li",
                      [
                        _c(
                          "router-link",
                          {
                            staticClass: "dropdown-item",
                            attrs: { to: "/soatransactions" }
                          },
                          [_c("a", [_vm._v("SOA Transaction")])]
                        )
                      ],
                      1
                    )
                  ]
                )
              ]),
              _vm._v(" "),
              _c("li", { staticClass: "nav-item dropdown" }, [
                _c(
                  "a",
                  {
                    staticClass: "nav-link dropdown-toggle",
                    attrs: {
                      href: "#",
                      id: "navbarDropdownMenuLink",
                      role: "button",
                      "data-toggle": "dropdown",
                      "aria-haspopup": "true",
                      "aria-expanded": "false",
                      "data-target": "#report"
                    }
                  },
                  [
                    _vm._v(
                      "\n                                Jeep Reports\n                                "
                    )
                  ]
                ),
                _vm._v(" "),
                _c(
                  "ul",
                  {
                    staticClass: "dropdown-menu",
                    attrs: {
                      "aria-labelledby": "navbarDropdownMenuLink",
                      id: "report"
                    }
                  },
                  [
                    _c(
                      "li",
                      [
                        _c(
                          "router-link",
                          {
                            staticClass: "dropdown-item",
                            attrs: { to: "/reportlistJVL" }
                          },
                          [_c("a", [_vm._v("Standard Jeep Report")])]
                        )
                      ],
                      1
                    ),
                    _vm._v(" "),
                    _c(
                      "li",
                      [
                        _c(
                          "router-link",
                          {
                            staticClass: "dropdown-item",
                            attrs: { to: "/reportlistJVLPerJeep" }
                          },
                          [
                            _c("a", [
                              _vm._v("Jeepney's Vehicle Log Billing Report")
                            ])
                          ]
                        )
                      ],
                      1
                    ),
                    _vm._v(" "),
                    _c(
                      "li",
                      [
                        _c(
                          "router-link",
                          {
                            staticClass: "dropdown-item",
                            attrs: { to: "/JeepSOATransmittal" }
                          },
                          [_c("a", [_vm._v("Jeepney's SOA Transmittal")])]
                        )
                      ],
                      1
                    )
                  ]
                )
              ])
            ])
          ]
        )
      ]
    ),
    _vm._v(" "),
    _c("div", { staticClass: "col-xs-12" }, [
      _c("div", { staticClass: "card" }, [
        _c("div", { staticClass: "card-header" }, [
          _vm._m(0),
          _vm._v(" "),
          _c("br"),
          _vm._v(" "),
          _c(
            "form",
            {
              staticStyle: { "border-style": "solid", "border-color": "coral" }
            },
            [
              _c("div", { staticClass: "form-inline" }, [
                _c(
                  "div",
                  {
                    staticClass: "input-group mb-3 input-group-sm",
                    staticStyle: { "margin-top": "10px", "margin-left": "5px" }
                  },
                  [
                    _vm._m(1),
                    _vm._v(" "),
                    _c("input", {
                      directives: [
                        {
                          name: "model",
                          rawName: "v-model",
                          value: _vm.DateFrom,
                          expression: "DateFrom"
                        }
                      ],
                      staticClass: "form-control",
                      attrs: { type: "date", name: "DateFrom" },
                      domProps: { value: _vm.DateFrom },
                      on: {
                        input: function($event) {
                          if ($event.target.composing) {
                            return
                          }
                          _vm.DateFrom = $event.target.value
                        }
                      }
                    })
                  ]
                ),
                _vm._v(" "),
                _c(
                  "div",
                  {
                    staticClass: "input-group mb-3 input-group-sm",
                    staticStyle: { "margin-top": "10px", "margin-left": "5px" }
                  },
                  [
                    _vm._m(2),
                    _vm._v(" "),
                    _c("input", {
                      directives: [
                        {
                          name: "model",
                          rawName: "v-model",
                          value: _vm.DateTo,
                          expression: "DateTo"
                        }
                      ],
                      staticClass: "form-control",
                      attrs: { type: "date", name: "DateTo" },
                      domProps: { value: _vm.DateTo },
                      on: {
                        input: function($event) {
                          if ($event.target.composing) {
                            return
                          }
                          _vm.DateTo = $event.target.value
                        }
                      }
                    })
                  ]
                )
              ])
            ]
          ),
          _vm._v(" "),
          _c("div", { staticClass: "card-tools" }),
          _vm._v(" "),
          _c("br")
        ]),
        _vm._v(" "),
        _c(
          "div",
          { staticClass: "card-body table-responsive pre-scrollable" },
          [
            _c("table", { staticClass: "table table-hover" }, [
              _c("thead", [
                _c("tr", [
                  _c("th", [
                    _c("input", {
                      directives: [
                        {
                          name: "model",
                          rawName: "v-model",
                          value: _vm.allSelected,
                          expression: "allSelected"
                        }
                      ],
                      attrs: { type: "checkbox" },
                      domProps: {
                        checked: Array.isArray(_vm.allSelected)
                          ? _vm._i(_vm.allSelected, null) > -1
                          : _vm.allSelected
                      },
                      on: {
                        click: _vm.selectAll,
                        change: function($event) {
                          var $$a = _vm.allSelected,
                            $$el = $event.target,
                            $$c = $$el.checked ? true : false
                          if (Array.isArray($$a)) {
                            var $$v = null,
                              $$i = _vm._i($$a, $$v)
                            if ($$el.checked) {
                              $$i < 0 && (_vm.allSelected = $$a.concat([$$v]))
                            } else {
                              $$i > -1 &&
                                (_vm.allSelected = $$a
                                  .slice(0, $$i)
                                  .concat($$a.slice($$i + 1)))
                            }
                          } else {
                            _vm.allSelected = $$c
                          }
                        }
                      }
                    })
                  ]),
                  _vm._v(" "),
                  _c("th", [_vm._v("SOA Date")]),
                  _vm._v(" "),
                  _c("th", [_vm._v("SOA No")]),
                  _vm._v(" "),
                  _c("th", [_vm._v("Billed TO")]),
                  _vm._v(" "),
                  _c("th", [_vm._v("Prepared By")]),
                  _vm._v(" "),
                  _c("th", [_vm._v("Amount")]),
                  _vm._v(" "),
                  _c("th", [_vm._v("Status")])
                ])
              ]),
              _vm._v(" "),
              _c(
                "tbody",
                _vm._l(_vm.filteredBlogs, function(soa) {
                  return _c(
                    "tr",
                    {
                      key: soa.Soa_JeepID,
                      staticStyle: { cursor: "pointer" },
                      attrs: { id: "element" },
                      on: {
                        click: function($event) {
                          return _vm.individualSOA(soa)
                        }
                      }
                    },
                    [
                      _c("td", [
                        _c("input", {
                          directives: [
                            {
                              name: "model",
                              rawName: "v-model",
                              value: _vm.checkedNames,
                              expression: "checkedNames"
                            }
                          ],
                          attrs: { type: "checkbox" },
                          domProps: {
                            value: soa.Soa_JeepID,
                            checked: Array.isArray(_vm.checkedNames)
                              ? _vm._i(_vm.checkedNames, soa.Soa_JeepID) > -1
                              : _vm.checkedNames
                          },
                          on: {
                            change: function($event) {
                              var $$a = _vm.checkedNames,
                                $$el = $event.target,
                                $$c = $$el.checked ? true : false
                              if (Array.isArray($$a)) {
                                var $$v = soa.Soa_JeepID,
                                  $$i = _vm._i($$a, $$v)
                                if ($$el.checked) {
                                  $$i < 0 &&
                                    (_vm.checkedNames = $$a.concat([$$v]))
                                } else {
                                  $$i > -1 &&
                                    (_vm.checkedNames = $$a
                                      .slice(0, $$i)
                                      .concat($$a.slice($$i + 1)))
                                }
                              } else {
                                _vm.checkedNames = $$c
                              }
                            }
                          }
                        })
                      ]),
                      _vm._v(" "),
                      _c("td", [_vm._v(_vm._s(soa.JeepSoaDate))]),
                      _vm._v(" "),
                      _c("td", [_vm._v(_vm._s(soa.GSCSoaSeriesNo))]),
                      _vm._v(" "),
                      _c("td", [_vm._v(_vm._s(soa.BilledName))]),
                      _vm._v(" "),
                      _c("td", [_vm._v(_vm._s(soa.PreparedBy))]),
                      _vm._v(" "),
                      _c("td", [_vm._v(_vm._s("Php" + soa.TotalAmount))]),
                      _vm._v(" "),
                      _c("td", [_vm._v(_vm._s(soa.Status))])
                    ]
                  )
                }),
                0
              )
            ])
          ]
        ),
        _vm._v(" "),
        _c("div", { staticClass: "card-footer" }, [
          _c("div", { staticClass: "row" }, [
            _c(
              "div",
              { staticClass: "col" },
              [
                _c(
                  "button",
                  {
                    staticClass: "btn btn-warning",
                    on: { click: _vm.soasetpaid }
                  },
                  [
                    _vm._v(
                      "\n                    Set As PAID/COLLECTED\n                    "
                    ),
                    _c("i", { staticClass: "fa fa-search fa fw" })
                  ]
                ),
                _vm._v(" "),
                _c(
                  "router-link",
                  {
                    staticClass: "btn btn-primary",
                    attrs: {
                      to: "/createsoa",
                      tag: "button",
                      "active-class": "active",
                      exact: ""
                    }
                  },
                  [
                    _vm._v(
                      "\n                    Create Jeep SOA Transaction Form\n                    "
                    ),
                    _c("i", { staticClass: "fa fa-user-plus fa fw" })
                  ]
                )
              ],
              1
            )
          ])
        ])
      ])
    ]),
    _vm._v(" "),
    _c(
      "div",
      {
        staticClass: "modal fade",
        attrs: {
          id: "addNew",
          tabindex: "-1",
          role: "dialog",
          "aria-labelledby": "addNewLabel",
          "aria-hidden": "true"
        }
      },
      [
        _c(
          "div",
          { staticClass: "modal-dialog", attrs: { role: "document" } },
          [
            _c("div", { staticClass: "modal-content" }, [
              _c("div", { staticClass: "modal-header" }, [
                _c(
                  "h5",
                  { staticClass: "modal-title", attrs: { id: "addNewLabel" } },
                  [
                    _vm._v("SOA Number "),
                    _c("b", [_vm._v(_vm._s(_vm.formSOA.GSCSoaSeriesNo))]),
                    _vm._v(":")
                  ]
                ),
                _vm._v(" "),
                _vm._m(3)
              ]),
              _vm._v(" "),
              _c(
                "div",
                { staticClass: "modal-body" },
                [
                  _c("center", [
                    _c(
                      "button",
                      {
                        staticClass: "btn btn-warning",
                        attrs: { type: "button" },
                        on: { click: _vm.detailView }
                      },
                      [_vm._v("View Detail")]
                    ),
                    _vm._v(" "),
                    _c(
                      "button",
                      {
                        staticClass: "btn btn-primary",
                        attrs: { type: "button" },
                        on: { click: _vm.viewSOAForm }
                      },
                      [_vm._v("View SOA Form")]
                    ),
                    _vm._v(" "),
                    _c(
                      "button",
                      {
                        staticClass: "btn btn-danger",
                        attrs: { type: "button" },
                        on: { click: _vm.cancelSOA }
                      },
                      [_vm._v("Cancel SOA")]
                    ),
                    _vm._v(" "),
                    _c(
                      "button",
                      {
                        staticClass: "btn btn-success",
                        attrs: { type: "button" },
                        on: { click: _vm.transmittalSOA }
                      },
                      [_vm._v("For Transmittal")]
                    )
                  ])
                ],
                1
              )
            ])
          ]
        )
      ]
    ),
    _vm._v(" "),
    _c(
      "div",
      {
        staticClass: "modal fade",
        attrs: {
          id: "addNewMultiple",
          tabindex: "-1",
          role: "dialog",
          "aria-labelledby": "addNewLabel",
          "aria-hidden": "true"
        }
      },
      [
        _c(
          "div",
          {
            staticClass: "modal-dialog modal-xl",
            staticStyle: { "overflow-y": "initial !important" },
            attrs: { role: "document" }
          },
          [
            _c("div", { staticClass: "modal-content" }, [
              _c("div", { staticClass: "modal-header" }, [
                _c(
                  "h5",
                  {
                    directives: [
                      {
                        name: "show",
                        rawName: "v-show",
                        value: !_vm.editmode,
                        expression: "!editmode"
                      }
                    ],
                    staticClass: "modal-title",
                    attrs: { id: "addNewLabel" }
                  },
                  [_vm._v("Jeep SOA Form")]
                ),
                _vm._v(" "),
                _c(
                  "h5",
                  {
                    directives: [
                      {
                        name: "show",
                        rawName: "v-show",
                        value: _vm.editmode,
                        expression: "editmode"
                      }
                    ],
                    staticClass: "modal-title",
                    attrs: { id: "addNewLabel" }
                  },
                  [_vm._v("Update Jeep SOA Form")]
                ),
                _vm._v(" "),
                _vm._m(4)
              ]),
              _vm._v(" "),
              _c(
                "form",
                {
                  on: {
                    submit: function($event) {
                      $event.preventDefault()
                      _vm.editmode ? _vm.updateJSOA() : _vm.createJSOA()
                    }
                  }
                },
                [
                  _c(
                    "div",
                    {
                      staticClass: "modal-body",
                      staticStyle: { height: "450px", "overflow-y": "auto" }
                    },
                    [
                      _c("div", { staticClass: "form-inline" }, [
                        _c(
                          "div",
                          {
                            staticClass: "input-group mb-3 input-group-sm",
                            staticStyle: { "margin-left": "5px" }
                          },
                          [
                            _vm._m(5),
                            _vm._v(" "),
                            _c("input", {
                              directives: [
                                {
                                  name: "model",
                                  rawName: "v-model",
                                  value: _vm.formSOA.Soa_JeepID,
                                  expression: "formSOA.Soa_JeepID"
                                }
                              ],
                              staticClass: "form-control",
                              attrs: { type: "text", "data-readonly": "" },
                              domProps: { value: _vm.formSOA.Soa_JeepID },
                              on: {
                                input: function($event) {
                                  if ($event.target.composing) {
                                    return
                                  }
                                  _vm.$set(
                                    _vm.formSOA,
                                    "Soa_JeepID",
                                    $event.target.value
                                  )
                                }
                              }
                            }),
                            _vm._v(" "),
                            _c(
                              "b-input-group-append",
                              [
                                _c(
                                  "b-button",
                                  {
                                    attrs: {
                                      variant: "outline-primary",
                                      size: "sm"
                                    },
                                    on: {
                                      click: function($event) {
                                        return _vm.searchSOAFunction()
                                      }
                                    }
                                  },
                                  [
                                    _c("i", {
                                      staticClass: "fa fa-search",
                                      attrs: { "aria-hidden": "true" }
                                    })
                                  ]
                                )
                              ],
                              1
                            )
                          ],
                          1
                        ),
                        _vm._v(" "),
                        _c(
                          "div",
                          {
                            staticClass: "input-group mb-3 input-group-sm",
                            staticStyle: { "margin-left": "5px" }
                          },
                          [
                            _vm._m(6),
                            _vm._v(" "),
                            _c("input", {
                              directives: [
                                {
                                  name: "model",
                                  rawName: "v-model",
                                  value: _vm.formSOA.JeepSoaDate,
                                  expression: "formSOA.JeepSoaDate"
                                }
                              ],
                              staticClass: "form-control",
                              attrs: {
                                id: "JeepSoaDate",
                                type: "date",
                                required: ""
                              },
                              domProps: { value: _vm.formSOA.JeepSoaDate },
                              on: {
                                input: function($event) {
                                  if ($event.target.composing) {
                                    return
                                  }
                                  _vm.$set(
                                    _vm.formSOA,
                                    "JeepSoaDate",
                                    $event.target.value
                                  )
                                }
                              }
                            })
                          ]
                        ),
                        _vm._v(" "),
                        _c(
                          "div",
                          {
                            staticClass: "input-group mb-3 input-group-sm",
                            staticStyle: { "margin-left": "5px" }
                          },
                          [
                            _vm._m(7),
                            _vm._v(" "),
                            _c("input", {
                              directives: [
                                {
                                  name: "model",
                                  rawName: "v-model",
                                  value: _vm.formSOA.Status,
                                  expression: "formSOA.Status"
                                }
                              ],
                              staticClass: "form-control",
                              attrs: {
                                type: "text",
                                name: "Status",
                                "data-readonly": ""
                              },
                              domProps: { value: _vm.formSOA.Status },
                              on: {
                                input: function($event) {
                                  if ($event.target.composing) {
                                    return
                                  }
                                  _vm.$set(
                                    _vm.formSOA,
                                    "Status",
                                    $event.target.value
                                  )
                                }
                              }
                            })
                          ]
                        ),
                        _vm._v(" "),
                        _c(
                          "div",
                          {
                            staticClass: "input-group mb-3 input-group-sm",
                            staticStyle: {
                              "text-transform": "uppercase",
                              "margin-left": "5px"
                            }
                          },
                          [
                            _vm._m(8),
                            _vm._v(" "),
                            _c("input", {
                              directives: [
                                {
                                  name: "model",
                                  rawName: "v-model",
                                  value: _vm.formSOA.GSCSoaSeriesNo,
                                  expression: "formSOA.GSCSoaSeriesNo"
                                }
                              ],
                              staticClass: "form-control",
                              attrs: { type: "text", name: "GSCSoaSeriesNo" },
                              domProps: { value: _vm.formSOA.GSCSoaSeriesNo },
                              on: {
                                input: function($event) {
                                  if ($event.target.composing) {
                                    return
                                  }
                                  _vm.$set(
                                    _vm.formSOA,
                                    "GSCSoaSeriesNo",
                                    $event.target.value
                                  )
                                }
                              }
                            })
                          ]
                        ),
                        _vm._v(" "),
                        _c(
                          "div",
                          {
                            staticClass: "input-group mb-3 input-group-sm",
                            staticStyle: {
                              "margin-left": "5px",
                              "text-transform": "uppercase"
                            }
                          },
                          [
                            _vm._m(9),
                            _vm._v(" "),
                            _c("input", {
                              directives: [
                                {
                                  name: "model",
                                  rawName: "v-model",
                                  value: _vm.formSOA.BilledName,
                                  expression: "formSOA.BilledName"
                                }
                              ],
                              staticClass: "form-control",
                              attrs: {
                                type: "text",
                                name: "BilledName",
                                readonly: ""
                              },
                              domProps: { value: _vm.formSOA.BilledName },
                              on: {
                                input: function($event) {
                                  if ($event.target.composing) {
                                    return
                                  }
                                  _vm.$set(
                                    _vm.formSOA,
                                    "BilledName",
                                    $event.target.value
                                  )
                                }
                              }
                            }),
                            _vm._v(" "),
                            _c("input", {
                              directives: [
                                {
                                  name: "model",
                                  rawName: "v-model",
                                  value: _vm.formSOA.BilledToIDLink,
                                  expression: "formSOA.BilledToIDLink"
                                }
                              ],
                              staticClass: "form-control",
                              attrs: {
                                type: "hidden",
                                name: "BilledToIDLink",
                                placeholder: "BilledToIDLink"
                              },
                              domProps: { value: _vm.formSOA.BilledToIDLink },
                              on: {
                                input: function($event) {
                                  if ($event.target.composing) {
                                    return
                                  }
                                  _vm.$set(
                                    _vm.formSOA,
                                    "BilledToIDLink",
                                    $event.target.value
                                  )
                                }
                              }
                            }),
                            _vm._v(" "),
                            _c(
                              "b-input-group-append",
                              [
                                _c(
                                  "b-button",
                                  {
                                    attrs: {
                                      variant: "outline-primary",
                                      size: "sm"
                                    },
                                    on: {
                                      click: function($event) {
                                        return _vm.searchClientSOAFunction()
                                      }
                                    }
                                  },
                                  [
                                    _c("i", {
                                      staticClass: "fa fa-search",
                                      attrs: { "aria-hidden": "true" }
                                    })
                                  ]
                                )
                              ],
                              1
                            )
                          ],
                          1
                        ),
                        _vm._v(" "),
                        _c(
                          "div",
                          {
                            staticClass: "input-group mb-3 input-group-sm",
                            staticStyle: {
                              "text-transform": "uppercase",
                              "margin-left": "5px"
                            }
                          },
                          [
                            _vm._m(10),
                            _vm._v(" "),
                            _c("textarea", {
                              directives: [
                                {
                                  name: "model",
                                  rawName: "v-model",
                                  value: _vm.formSOA.BilledAddress,
                                  expression: "formSOA.BilledAddress"
                                }
                              ],
                              staticClass: "form-control",
                              attrs: { type: "text", name: "BilledAddress" },
                              domProps: { value: _vm.formSOA.BilledAddress },
                              on: {
                                input: function($event) {
                                  if ($event.target.composing) {
                                    return
                                  }
                                  _vm.$set(
                                    _vm.formSOA,
                                    "BilledAddress",
                                    $event.target.value
                                  )
                                }
                              }
                            })
                          ]
                        ),
                        _vm._v(" "),
                        _c(
                          "div",
                          {
                            staticClass: "input-group mb-3 input-group-sm",
                            staticStyle: { "margin-left": "5px" }
                          },
                          [
                            _vm._m(11),
                            _vm._v(" "),
                            _c("input", {
                              directives: [
                                {
                                  name: "model",
                                  rawName: "v-model",
                                  value: _vm.formSOA.TotalAmount,
                                  expression: "formSOA.TotalAmount"
                                }
                              ],
                              staticClass: "form-control",
                              attrs: { type: "text", name: "TotalAmount" },
                              domProps: { value: _vm.formSOA.TotalAmount },
                              on: {
                                input: function($event) {
                                  if ($event.target.composing) {
                                    return
                                  }
                                  _vm.$set(
                                    _vm.formSOA,
                                    "TotalAmount",
                                    $event.target.value
                                  )
                                }
                              }
                            })
                          ]
                        ),
                        _vm._v(" "),
                        _c(
                          "div",
                          {
                            staticClass: "input-group mb-3 input-group-sm",
                            staticStyle: { "margin-left": "5px" }
                          },
                          [
                            _vm._m(12),
                            _vm._v(" "),
                            _c("input", {
                              directives: [
                                {
                                  name: "model",
                                  rawName: "v-model",
                                  value: _vm.formSOA.THRUName,
                                  expression: "formSOA.THRUName"
                                }
                              ],
                              staticClass: "form-control",
                              attrs: {
                                type: "text",
                                name: "THRUName",
                                readonly: ""
                              },
                              domProps: { value: _vm.formSOA.THRUName },
                              on: {
                                input: function($event) {
                                  if ($event.target.composing) {
                                    return
                                  }
                                  _vm.$set(
                                    _vm.formSOA,
                                    "THRUName",
                                    $event.target.value
                                  )
                                }
                              }
                            }),
                            _vm._v(" "),
                            _c(
                              "b-input-group-append",
                              [
                                _c(
                                  "b-button",
                                  {
                                    attrs: {
                                      variant: "outline-primary",
                                      size: "sm"
                                    },
                                    on: {
                                      click: function($event) {
                                        return _vm.searchSignatoryTHRUSOAFunction()
                                      }
                                    }
                                  },
                                  [
                                    _c("i", {
                                      staticClass: "fa fa-search",
                                      attrs: { "aria-hidden": "true" }
                                    })
                                  ]
                                )
                              ],
                              1
                            )
                          ],
                          1
                        ),
                        _vm._v(" "),
                        _c(
                          "div",
                          {
                            staticClass: "input-group mb-3 input-group-sm",
                            staticStyle: { "margin-left": "5px" }
                          },
                          [
                            _vm._m(13),
                            _vm._v(" "),
                            _c("input", {
                              directives: [
                                {
                                  name: "model",
                                  rawName: "v-model",
                                  value: _vm.formSOA.THRUPos,
                                  expression: "formSOA.THRUPos"
                                }
                              ],
                              staticClass: "form-control",
                              attrs: {
                                type: "text",
                                name: "THRUPos",
                                readonly: ""
                              },
                              domProps: { value: _vm.formSOA.THRUPos },
                              on: {
                                input: function($event) {
                                  if ($event.target.composing) {
                                    return
                                  }
                                  _vm.$set(
                                    _vm.formSOA,
                                    "THRUPos",
                                    $event.target.value
                                  )
                                }
                              }
                            })
                          ]
                        ),
                        _vm._v(" "),
                        _c(
                          "div",
                          {
                            staticClass: "input-group mb-3 input-group-sm",
                            staticStyle: { "margin-left": "5px" }
                          },
                          [
                            _vm._m(14),
                            _vm._v(" "),
                            _c("input", {
                              directives: [
                                {
                                  name: "model",
                                  rawName: "v-model",
                                  value: _vm.formSOA.Salutation,
                                  expression: "formSOA.Salutation"
                                }
                              ],
                              staticClass: "form-control",
                              attrs: {
                                type: "text",
                                name: "Salutation",
                                required: ""
                              },
                              domProps: { value: _vm.formSOA.Salutation },
                              on: {
                                input: function($event) {
                                  if ($event.target.composing) {
                                    return
                                  }
                                  _vm.$set(
                                    _vm.formSOA,
                                    "Salutation",
                                    $event.target.value
                                  )
                                }
                              }
                            })
                          ]
                        ),
                        _vm._v(" "),
                        _c(
                          "div",
                          {
                            staticClass: "input-group mb-3 input-group-sm",
                            staticStyle: { "margin-left": "5px" }
                          },
                          [
                            _vm._m(15),
                            _vm._v(" "),
                            _c(
                              "div",
                              {
                                staticClass: "form-control",
                                attrs: {
                                  contenteditable: "true",
                                  id: "stringinput"
                                },
                                domProps: {
                                  innerHTML: _vm._s(_vm.formSOA.BodyofLetter)
                                },
                                on: { input: _vm.fakeVmodel }
                              },
                              [
                                _vm._v(
                                  "     " + _vm._s(_vm.formSOA.BodyofLetter)
                                )
                              ]
                            )
                          ]
                        )
                      ]),
                      _vm._v(" "),
                      _c("div", { staticClass: "col-xs-12" }, [
                        _c("div", { staticClass: "row" }, [
                          _c("div", { staticClass: "col" }, [
                            _c(
                              "div",
                              {
                                staticClass: "input-group mb-3 input-group-sm"
                              },
                              [
                                _vm._m(16),
                                _vm._v(" "),
                                _c("input", {
                                  directives: [
                                    {
                                      name: "model",
                                      rawName: "v-model",
                                      value: _vm.formSOA.PreparedBy,
                                      expression: "formSOA.PreparedBy"
                                    }
                                  ],
                                  staticClass: "form-control",
                                  attrs: {
                                    type: "text",
                                    name: "PreparedBy",
                                    readonly: ""
                                  },
                                  domProps: { value: _vm.formSOA.PreparedBy },
                                  on: {
                                    input: function($event) {
                                      if ($event.target.composing) {
                                        return
                                      }
                                      _vm.$set(
                                        _vm.formSOA,
                                        "PreparedBy",
                                        $event.target.value
                                      )
                                    }
                                  }
                                }),
                                _vm._v(" "),
                                _c(
                                  "b-input-group-append",
                                  [
                                    _c(
                                      "b-button",
                                      {
                                        attrs: {
                                          variant: "outline-primary",
                                          size: "sm"
                                        },
                                        on: {
                                          click: function($event) {
                                            return _vm.searchSignatoryPreparedSOAFunction()
                                          }
                                        }
                                      },
                                      [
                                        _c("i", {
                                          staticClass: "fa fa-search",
                                          attrs: { "aria-hidden": "true" }
                                        })
                                      ]
                                    )
                                  ],
                                  1
                                )
                              ],
                              1
                            )
                          ]),
                          _vm._v(" "),
                          _c("div", { staticClass: "col" }, [
                            _c(
                              "div",
                              {
                                staticClass: "input-group mb-3 input-group-sm"
                              },
                              [
                                _vm._m(17),
                                _vm._v(" "),
                                _c("input", {
                                  directives: [
                                    {
                                      name: "model",
                                      rawName: "v-model",
                                      value: _vm.formSOA.CheckedBy,
                                      expression: "formSOA.CheckedBy"
                                    }
                                  ],
                                  staticClass: "form-control",
                                  attrs: {
                                    type: "text",
                                    name: "CheckedBy",
                                    readonly: ""
                                  },
                                  domProps: { value: _vm.formSOA.CheckedBy },
                                  on: {
                                    input: function($event) {
                                      if ($event.target.composing) {
                                        return
                                      }
                                      _vm.$set(
                                        _vm.formSOA,
                                        "CheckedBy",
                                        $event.target.value
                                      )
                                    }
                                  }
                                }),
                                _vm._v(" "),
                                _c(
                                  "b-input-group-append",
                                  [
                                    _c(
                                      "b-button",
                                      {
                                        attrs: {
                                          variant: "outline-primary",
                                          size: "sm"
                                        },
                                        on: {
                                          click: function($event) {
                                            return _vm.searchSignatoryCheckedSOAFunction()
                                          }
                                        }
                                      },
                                      [
                                        _c("i", {
                                          staticClass: "fa fa-search",
                                          attrs: { "aria-hidden": "true" }
                                        })
                                      ]
                                    )
                                  ],
                                  1
                                )
                              ],
                              1
                            )
                          ]),
                          _vm._v(" "),
                          _c("div", { staticClass: "col" }, [
                            _c(
                              "div",
                              {
                                staticClass: "input-group mb-3 input-group-sm"
                              },
                              [
                                _vm._m(18),
                                _vm._v(" "),
                                _c("input", {
                                  directives: [
                                    {
                                      name: "model",
                                      rawName: "v-model",
                                      value: _vm.formSOA.NotedBy,
                                      expression: "formSOA.NotedBy"
                                    }
                                  ],
                                  staticClass: "form-control",
                                  attrs: {
                                    type: "text",
                                    name: "NotedBy",
                                    readonly: ""
                                  },
                                  domProps: { value: _vm.formSOA.NotedBy },
                                  on: {
                                    input: function($event) {
                                      if ($event.target.composing) {
                                        return
                                      }
                                      _vm.$set(
                                        _vm.formSOA,
                                        "NotedBy",
                                        $event.target.value
                                      )
                                    }
                                  }
                                }),
                                _vm._v(" "),
                                _c(
                                  "b-input-group-append",
                                  [
                                    _c(
                                      "b-button",
                                      {
                                        attrs: {
                                          variant: "outline-primary",
                                          size: "sm"
                                        },
                                        on: {
                                          click: function($event) {
                                            return _vm.searchSignatoryNotedSOAFunction()
                                          }
                                        }
                                      },
                                      [
                                        _c("i", {
                                          staticClass: "fa fa-search",
                                          attrs: { "aria-hidden": "true" }
                                        })
                                      ]
                                    )
                                  ],
                                  1
                                )
                              ],
                              1
                            )
                          ])
                        ]),
                        _vm._v(" "),
                        _c("div", { staticClass: "row" }, [
                          _c("div", { staticClass: "col" }, [
                            _c(
                              "div",
                              {
                                staticClass: "input-group mb-3 input-group-sm"
                              },
                              [
                                _vm._m(19),
                                _vm._v(" "),
                                _c("input", {
                                  directives: [
                                    {
                                      name: "model",
                                      rawName: "v-model",
                                      value: _vm.formSOA.PreparedByPos,
                                      expression: "formSOA.PreparedByPos"
                                    }
                                  ],
                                  staticClass: "form-control",
                                  attrs: {
                                    type: "text",
                                    name: "PreparedByPos",
                                    readonly: ""
                                  },
                                  domProps: {
                                    value: _vm.formSOA.PreparedByPos
                                  },
                                  on: {
                                    input: function($event) {
                                      if ($event.target.composing) {
                                        return
                                      }
                                      _vm.$set(
                                        _vm.formSOA,
                                        "PreparedByPos",
                                        $event.target.value
                                      )
                                    }
                                  }
                                })
                              ]
                            )
                          ]),
                          _vm._v(" "),
                          _c("div", { staticClass: "col" }, [
                            _c(
                              "div",
                              {
                                staticClass: "input-group mb-3 input-group-sm"
                              },
                              [
                                _c(
                                  "div",
                                  {
                                    staticClass:
                                      "input-group mb-3 input-group-sm"
                                  },
                                  [
                                    _vm._m(20),
                                    _vm._v(" "),
                                    _c("input", {
                                      directives: [
                                        {
                                          name: "model",
                                          rawName: "v-model",
                                          value: _vm.formSOA.CheckedByPos,
                                          expression: "formSOA.CheckedByPos"
                                        }
                                      ],
                                      staticClass: "form-control",
                                      attrs: {
                                        type: "text",
                                        name: "CheckedByPos",
                                        readonly: ""
                                      },
                                      domProps: {
                                        value: _vm.formSOA.CheckedByPos
                                      },
                                      on: {
                                        input: function($event) {
                                          if ($event.target.composing) {
                                            return
                                          }
                                          _vm.$set(
                                            _vm.formSOA,
                                            "CheckedByPos",
                                            $event.target.value
                                          )
                                        }
                                      }
                                    })
                                  ]
                                )
                              ]
                            )
                          ]),
                          _vm._v(" "),
                          _c("div", { staticClass: "col" }, [
                            _c(
                              "div",
                              {
                                staticClass: "input-group mb-3 input-group-sm"
                              },
                              [
                                _vm._m(21),
                                _vm._v(" "),
                                _c("input", {
                                  directives: [
                                    {
                                      name: "model",
                                      rawName: "v-model",
                                      value: _vm.formSOA.NotedByPos,
                                      expression: "formSOA.NotedByPos"
                                    }
                                  ],
                                  staticClass: "form-control",
                                  attrs: {
                                    type: "text",
                                    name: "NotedByPos",
                                    readonly: ""
                                  },
                                  domProps: { value: _vm.formSOA.NotedByPos },
                                  on: {
                                    input: function($event) {
                                      if ($event.target.composing) {
                                        return
                                      }
                                      _vm.$set(
                                        _vm.formSOA,
                                        "NotedByPos",
                                        $event.target.value
                                      )
                                    }
                                  }
                                })
                              ]
                            )
                          ])
                        ]),
                        _vm._v(" "),
                        _c("div", { staticClass: "row" }, [
                          _c("div", { staticClass: "col-md-4" }, [
                            _c(
                              "div",
                              {
                                staticClass: "input-group mb-3 input-group-sm"
                              },
                              [
                                _vm._m(22),
                                _vm._v(" "),
                                _c("input", {
                                  directives: [
                                    {
                                      name: "model",
                                      rawName: "v-model",
                                      value: _vm.formSOA.InvoiceNo,
                                      expression: "formSOA.InvoiceNo"
                                    }
                                  ],
                                  staticClass: "form-control",
                                  attrs: { type: "text", name: "InvoiceNo" },
                                  domProps: { value: _vm.formSOA.InvoiceNo },
                                  on: {
                                    input: function($event) {
                                      if ($event.target.composing) {
                                        return
                                      }
                                      _vm.$set(
                                        _vm.formSOA,
                                        "InvoiceNo",
                                        $event.target.value
                                      )
                                    }
                                  }
                                })
                              ]
                            )
                          ]),
                          _vm._v(" "),
                          _c("div", { staticClass: "col-md-4" }, [
                            _c(
                              "div",
                              {
                                staticClass: "input-group mb-3 input-group-sm"
                              },
                              [
                                _vm._m(23),
                                _vm._v(" "),
                                _c("input", {
                                  directives: [
                                    {
                                      name: "model",
                                      rawName: "v-model",
                                      value: _vm.formSOA.InvoiceDate,
                                      expression: "formSOA.InvoiceDate"
                                    }
                                  ],
                                  staticClass: "form-control",
                                  attrs: { type: "date", name: "InvoiceDate" },
                                  domProps: { value: _vm.formSOA.InvoiceDate },
                                  on: {
                                    input: function($event) {
                                      if ($event.target.composing) {
                                        return
                                      }
                                      _vm.$set(
                                        _vm.formSOA,
                                        "InvoiceDate",
                                        $event.target.value
                                      )
                                    }
                                  }
                                })
                              ]
                            )
                          ])
                        ])
                      ])
                    ]
                  ),
                  _vm._v(" "),
                  _c("div", { staticClass: "modal-footer" }, [
                    _c(
                      "button",
                      {
                        staticClass: "btn btn-danger",
                        attrs: { type: "button", "data-dismiss": "modal" }
                      },
                      [_vm._v("Close")]
                    ),
                    _vm._v(" "),
                    _c(
                      "button",
                      {
                        staticClass: "btn btn-primary",
                        attrs: { type: "button" },
                        on: { click: _vm.downloadPDFSOA }
                      },
                      [_vm._v("Print SOA")]
                    ),
                    _vm._v(" "),
                    _c(
                      "button",
                      {
                        directives: [
                          {
                            name: "show",
                            rawName: "v-show",
                            value: !_vm.editmode,
                            expression: "!editmode"
                          }
                        ],
                        staticClass: "btn btn-success",
                        attrs: { type: "submit" }
                      },
                      [_vm._v("Save")]
                    ),
                    _vm._v(" "),
                    _c(
                      "button",
                      {
                        directives: [
                          {
                            name: "show",
                            rawName: "v-show",
                            value: _vm.editmode,
                            expression: "editmode"
                          }
                        ],
                        staticClass: "btn btn-success",
                        attrs: { type: "submit" }
                      },
                      [_vm._v("Update")]
                    ),
                    _vm._v(" "),
                    _c(
                      "button",
                      {
                        directives: [
                          {
                            name: "show",
                            rawName: "v-show",
                            value: _vm.editmode,
                            expression: "editmode"
                          }
                        ],
                        staticClass: "btn btn-warning",
                        attrs: { type: "button" },
                        on: { click: _vm.soapost }
                      },
                      [_vm._v("Post")]
                    )
                  ])
                ]
              )
            ])
          ]
        )
      ]
    ),
    _vm._v(" "),
    _c(
      "div",
      {
        staticClass: "modal fade",
        attrs: {
          id: "viewdetailindividual",
          tabindex: "-1",
          role: "dialog",
          "aria-labelledby": "addNewLabel",
          "aria-hidden": "true"
        }
      },
      [
        _c(
          "div",
          { staticClass: "modal-dialog modal-xl", attrs: { role: "document" } },
          [
            _c("div", { staticClass: "modal-content" }, [
              _c("div", { staticClass: "modal-header" }, [
                _c(
                  "h5",
                  {
                    directives: [
                      {
                        name: "show",
                        rawName: "v-show",
                        value: !_vm.editmode,
                        expression: "!editmode"
                      }
                    ],
                    staticClass: "modal-title",
                    attrs: { id: "addNewLabel" }
                  },
                  [_vm._v("Jeep SOA Form")]
                ),
                _vm._v(" "),
                _c(
                  "h5",
                  {
                    directives: [
                      {
                        name: "show",
                        rawName: "v-show",
                        value: _vm.editmode,
                        expression: "editmode"
                      }
                    ],
                    staticClass: "modal-title",
                    attrs: { id: "addNewLabel" }
                  },
                  [_vm._v("Update Jeep SOA Form")]
                ),
                _vm._v(" "),
                _vm._m(24)
              ]),
              _vm._v(" "),
              _c("div", { staticClass: "modal-body" }, [
                _c("div", { staticClass: "table-responsive pre-scrollable" }, [
                  _c("table", { staticClass: "table table-hover" }, [
                    _vm._m(25),
                    _vm._v(" "),
                    _c(
                      "tbody",
                      _vm._l(_vm.detailSOAIndividual, function(jvl) {
                        return _c("tr", { key: jvl.JVLHDRID }, [
                          _c("td", [_vm._v(_vm._s(jvl.JVLDate))]),
                          _vm._v(" "),
                          _c("td", [_vm._v(_vm._s(jvl.OVLNo))]),
                          _vm._v(" "),
                          _c("td", [_vm._v(_vm._s(jvl.JeepPlateNo))]),
                          _vm._v(" "),
                          _c("td", [_vm._v(_vm._s(jvl.DriverName))]),
                          _vm._v(" "),
                          _c("td", [_vm._v(_vm._s(jvl.TruckerName))]),
                          _vm._v(" "),
                          _c("td", [_vm._v(_vm._s("Php" + jvl.BillAmount))]),
                          _vm._v(" "),
                          _c("td", [_vm._v(_vm._s(jvl.LessAdmin))]),
                          _vm._v(" "),
                          _c("td", [_vm._v(_vm._s(jvl.LessFuel))]),
                          _vm._v(" "),
                          _c("td", [_vm._v(_vm._s("Php" + jvl.NetTrucker))]),
                          _vm._v(" "),
                          _c("td", [_vm._v(_vm._s(jvl.Status))]),
                          _vm._v(" "),
                          _c("td", [_vm._v(_vm._s(jvl.SOANumber))]),
                          _vm._v(" "),
                          _c("td", [_vm._v(_vm._s(jvl.SOADate))]),
                          _vm._v(" "),
                          _c("td", [_vm._v(_vm._s(jvl.ChargeInvoiceNumber))])
                        ])
                      }),
                      0
                    )
                  ])
                ])
              ])
            ])
          ]
        )
      ]
    )
  ])
}
var staticRenderFns = [
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("h3", { staticClass: "card-title" }, [
      _c("b", [_vm._v("Previous Jeep SOA")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [
        _vm._v("Search Date From")
      ])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [
        _vm._v("Search Date To")
      ])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c(
      "button",
      {
        staticClass: "close",
        attrs: {
          type: "button",
          "data-dismiss": "modal",
          "aria-label": "Close"
        }
      },
      [_c("span", { attrs: { "aria-hidden": "true" } }, [_vm._v("×")])]
    )
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c(
      "button",
      {
        staticClass: "close",
        attrs: {
          type: "button",
          "data-dismiss": "modal",
          "aria-label": "Close"
        }
      },
      [_c("span", { attrs: { "aria-hidden": "true" } }, [_vm._v("×")])]
    )
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("SOA ID")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Date")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Status")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("SOA Number")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Billed To")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [
        _vm._v("Billed Address")
      ])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Total Amount")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("THRU Name")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("THRU Pos.")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Salutation")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [
        _vm._v("Body of Letter")
      ])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Prepared By:")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Checked By:")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Noted By:")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Prepared Pos.")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Checked Pos.")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Noted Pos.")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [
        _vm._v("Charge Invoice No.")
      ])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [
        _vm._v("Charge Invoice Date")
      ])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c(
      "button",
      {
        staticClass: "close",
        attrs: {
          type: "button",
          "data-dismiss": "modal",
          "aria-label": "Close"
        }
      },
      [_c("span", { attrs: { "aria-hidden": "true" } }, [_vm._v("×")])]
    )
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("thead", [
      _c("tr", [
        _c("th", [_vm._v("Date")]),
        _vm._v(" "),
        _c("th", [_vm._v("OVL No")]),
        _vm._v(" "),
        _c("th", [_vm._v("Plate No")]),
        _vm._v(" "),
        _c("th", [_vm._v("Driver")]),
        _vm._v(" "),
        _c("th", [_vm._v("Operator")]),
        _vm._v(" "),
        _c("th", [_vm._v("Billable Amt")]),
        _vm._v(" "),
        _c("th", [_vm._v("Less Admin")]),
        _vm._v(" "),
        _c("th", [_vm._v("Less Fuel")]),
        _vm._v(" "),
        _c("th", [_vm._v("Net Operator")]),
        _vm._v(" "),
        _c("th", [_vm._v("Status")]),
        _vm._v(" "),
        _c("th", [_vm._v("SOA No")]),
        _vm._v(" "),
        _c("th", [_vm._v("SOA Date")]),
        _vm._v(" "),
        _c("th", [_vm._v("Charge Invoice No")])
      ])
    ])
  }
]
render._withStripped = true



/***/ }),

/***/ "./resources/js/components/JeepComponents/JeepSOAList.vue":
/*!****************************************************************!*\
  !*** ./resources/js/components/JeepComponents/JeepSOAList.vue ***!
  \****************************************************************/
/*! exports provided: default */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _JeepSOAList_vue_vue_type_template_id_67b85895___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./JeepSOAList.vue?vue&type=template&id=67b85895& */ "./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=template&id=67b85895&");
/* harmony import */ var _JeepSOAList_vue_vue_type_script_lang_js___WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./JeepSOAList.vue?vue&type=script&lang=js& */ "./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=script&lang=js&");
/* empty/unused harmony star reexport *//* harmony import */ var _JeepSOAList_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./JeepSOAList.vue?vue&type=style&index=0&lang=css& */ "./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=style&index=0&lang=css&");
/* harmony import */ var _node_modules_vue_loader_lib_runtime_componentNormalizer_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../../../../node_modules/vue-loader/lib/runtime/componentNormalizer.js */ "./node_modules/vue-loader/lib/runtime/componentNormalizer.js");






/* normalize component */

var component = Object(_node_modules_vue_loader_lib_runtime_componentNormalizer_js__WEBPACK_IMPORTED_MODULE_3__["default"])(
  _JeepSOAList_vue_vue_type_script_lang_js___WEBPACK_IMPORTED_MODULE_1__["default"],
  _JeepSOAList_vue_vue_type_template_id_67b85895___WEBPACK_IMPORTED_MODULE_0__["render"],
  _JeepSOAList_vue_vue_type_template_id_67b85895___WEBPACK_IMPORTED_MODULE_0__["staticRenderFns"],
  false,
  null,
  null,
  null
  
)

/* hot reload */
if (false) { var api; }
component.options.__file = "resources/js/components/JeepComponents/JeepSOAList.vue"
/* harmony default export */ __webpack_exports__["default"] = (component.exports);

/***/ }),

/***/ "./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=script&lang=js&":
/*!*****************************************************************************************!*\
  !*** ./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=script&lang=js& ***!
  \*****************************************************************************************/
/*! exports provided: default */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _node_modules_babel_loader_lib_index_js_ref_4_0_node_modules_vue_loader_lib_index_js_vue_loader_options_JeepSOAList_vue_vue_type_script_lang_js___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! -!../../../../node_modules/babel-loader/lib??ref--4-0!../../../../node_modules/vue-loader/lib??vue-loader-options!./JeepSOAList.vue?vue&type=script&lang=js& */ "./node_modules/babel-loader/lib/index.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=script&lang=js&");
/* empty/unused harmony star reexport */ /* harmony default export */ __webpack_exports__["default"] = (_node_modules_babel_loader_lib_index_js_ref_4_0_node_modules_vue_loader_lib_index_js_vue_loader_options_JeepSOAList_vue_vue_type_script_lang_js___WEBPACK_IMPORTED_MODULE_0__["default"]); 

/***/ }),

/***/ "./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=style&index=0&lang=css&":
/*!*************************************************************************************************!*\
  !*** ./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=style&index=0&lang=css& ***!
  \*************************************************************************************************/
/*! no static exports found */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _node_modules_style_loader_index_js_node_modules_css_loader_index_js_ref_6_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_6_2_node_modules_vue_loader_lib_index_js_vue_loader_options_JeepSOAList_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! -!../../../../node_modules/style-loader!../../../../node_modules/css-loader??ref--6-1!../../../../node_modules/vue-loader/lib/loaders/stylePostLoader.js!../../../../node_modules/postcss-loader/src??ref--6-2!../../../../node_modules/vue-loader/lib??vue-loader-options!./JeepSOAList.vue?vue&type=style&index=0&lang=css& */ "./node_modules/style-loader/index.js!./node_modules/css-loader/index.js?!./node_modules/vue-loader/lib/loaders/stylePostLoader.js!./node_modules/postcss-loader/src/index.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=style&index=0&lang=css&");
/* harmony import */ var _node_modules_style_loader_index_js_node_modules_css_loader_index_js_ref_6_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_6_2_node_modules_vue_loader_lib_index_js_vue_loader_options_JeepSOAList_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_node_modules_style_loader_index_js_node_modules_css_loader_index_js_ref_6_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_6_2_node_modules_vue_loader_lib_index_js_vue_loader_options_JeepSOAList_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_0__);
/* harmony reexport (unknown) */ for(var __WEBPACK_IMPORT_KEY__ in _node_modules_style_loader_index_js_node_modules_css_loader_index_js_ref_6_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_6_2_node_modules_vue_loader_lib_index_js_vue_loader_options_JeepSOAList_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_0__) if(__WEBPACK_IMPORT_KEY__ !== 'default') (function(key) { __webpack_require__.d(__webpack_exports__, key, function() { return _node_modules_style_loader_index_js_node_modules_css_loader_index_js_ref_6_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_6_2_node_modules_vue_loader_lib_index_js_vue_loader_options_JeepSOAList_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_0__[key]; }) }(__WEBPACK_IMPORT_KEY__));
 /* harmony default export */ __webpack_exports__["default"] = (_node_modules_style_loader_index_js_node_modules_css_loader_index_js_ref_6_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_6_2_node_modules_vue_loader_lib_index_js_vue_loader_options_JeepSOAList_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_0___default.a); 

/***/ }),

/***/ "./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=template&id=67b85895&":
/*!***********************************************************************************************!*\
  !*** ./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=template&id=67b85895& ***!
  \***********************************************************************************************/
/*! exports provided: render, staticRenderFns */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _node_modules_vue_loader_lib_loaders_templateLoader_js_vue_loader_options_node_modules_vue_loader_lib_index_js_vue_loader_options_JeepSOAList_vue_vue_type_template_id_67b85895___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! -!../../../../node_modules/vue-loader/lib/loaders/templateLoader.js??vue-loader-options!../../../../node_modules/vue-loader/lib??vue-loader-options!./JeepSOAList.vue?vue&type=template&id=67b85895& */ "./node_modules/vue-loader/lib/loaders/templateLoader.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/JeepComponents/JeepSOAList.vue?vue&type=template&id=67b85895&");
/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "render", function() { return _node_modules_vue_loader_lib_loaders_templateLoader_js_vue_loader_options_node_modules_vue_loader_lib_index_js_vue_loader_options_JeepSOAList_vue_vue_type_template_id_67b85895___WEBPACK_IMPORTED_MODULE_0__["render"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "staticRenderFns", function() { return _node_modules_vue_loader_lib_loaders_templateLoader_js_vue_loader_options_node_modules_vue_loader_lib_index_js_vue_loader_options_JeepSOAList_vue_vue_type_template_id_67b85895___WEBPACK_IMPORTED_MODULE_0__["staticRenderFns"]; });



/***/ })

}]);