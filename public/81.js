(window["webpackJsonp"] = window["webpackJsonp"] || []).push([[81],{

/***/ "./node_modules/babel-loader/lib/index.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=script&lang=js&":
/*!**************************************************************************************************************************************************************************************!*\
  !*** ./node_modules/babel-loader/lib??ref--4-0!./node_modules/vue-loader/lib??vue-loader-options!./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=script&lang=js& ***!
  \**************************************************************************************************************************************************************************************/
/*! exports provided: default */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var moment__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! moment */ "./node_modules/moment/moment.js");
/* harmony import */ var moment__WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(moment__WEBPACK_IMPORTED_MODULE_0__);
/* harmony import */ var _search_SearchJeep_SearchSignatorys_vue__WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ../search/SearchJeep/SearchSignatorys.vue */ "./resources/js/components/search/SearchJeep/SearchSignatorys.vue");
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//
//


/* harmony default export */ __webpack_exports__["default"] = ({
  components: {
    'search-signatorys': _search_SearchJeep_SearchSignatorys_vue__WEBPACK_IMPORTED_MODULE_1__["default"]
  },
  data: function data() {
    return {
      uri: 'data:application/vnd.ms-excel;base64,',
      template: '<html xmlns:o="urn:schemas-microsoft-com:office:office" xmlns:x="urn:schemas-microsoft-com:office:excel" xmlns="http://www.w3.org/TR/REC-html40"><head><!--[if gte mso 9]><xml><x:ExcelWorkbook><x:ExcelWorksheets><x:ExcelWorksheet><x:Name>{worksheet}</x:Name><x:WorksheetOptions><x:DisplayGridlines/></x:WorksheetOptions></x:ExcelWorksheet></x:ExcelWorksheets></x:ExcelWorkbook></xml><![endif]--></head><body><table>{table}</table></body></html>',
      base64: function base64(s) {
        return window.btoa(unescape(encodeURIComponent(s)));
      },
      format: function format(s, c) {
        return s.replace(/{(\w+)}/g, function (m, p) {
          return c[p];
        });
      },
      sum: '',
      signator: '',
      checkedNames: [],
      filteredblogs: [],
      csoatotals: [],
      checkedNamesFilter: [],
      editmode: false,
      equalequal: false,
      allSelected: false,
      jvls: [],
      jvlsobject: {},
      search: '',
      jvcps: {},
      drivers: [],
      vehicles: {},
      rates: {},
      operators: {},
      batch: [],
      jvlfilter: [],
      jvlbalamt: [],
      var1: '',
      var2: '',
      first: '',
      second: '',
      eq: 'true',
      truevalue: '',
      falsevalue: '',
      SearchJeepPlateNo: '',
      DateFrom: '',
      DateTo: '',
      DateToday: '',
      JeepVehicleCollectionPayments: {},
      form: new Form({
        JVLHDRID: '',
        JVLHDRIDFilter: '',
        JVLDate: '',
        OVLNo: '',
        JeepIDLink: '',
        JeepPlateNo: '',
        DriverIDLink: '',
        DriverLastName: '',
        DriverFirstName: '',
        DriverMiddleName: '',
        DriverExtName: '',
        TruckerIDLink: '',
        TruckerLastName: '',
        TruckerFirstName: '',
        TruckerMiddleName: '',
        TruckerExtName: '',
        BillAmount: '',
        LessAdmin: '',
        LessFuel: '',
        NetTrucker: '',
        Status: '',
        SOANumber: '',
        SOADate: '',
        ChargeInvoiceNumber: '',
        GLCode: '',
        CostCenter: '',
        PerDayRate: '',
        NumberofDays: '',
        DriverName: '',
        TruckerName: '',
        CollectedAmount: '',
        CollectedAmountHDR: '',
        BalanceAmountHDR: '',
        BalanceAmount: '',
        ORCRNumber: '',
        ORCRDate: '',
        Remarks: '',
        JVLHDRID_Link: '',
        TotalAmount: ''
      }),
      formSOA: new Form({
        Soa_JeepID: '',
        GSCSoaSeriesNo: '',
        Status: '',
        BilledToIDLink: '',
        BilledName: '',
        BilledAddress: '',
        THRUName: '',
        THRUPos: '',
        JeepSoaDate: '',
        PreparedBy: '',
        PreparedByPos: '',
        CheckedBy: '',
        CheckedByPos: '',
        NotedBy: '',
        NotedByPos: '',
        ReceivedBy: '',
        ReceivedByPos: ''
      })
    };
  },
  methods: {
    updateTitleSignatory: function updateTitleSignatory(_updateTitleSignatory) {
      var fname = '';
      var mname = '';
      var lname = '';
      var ename = '';

      if (_updateTitleSignatory.fname === null) {
        fname = '';
      } else {
        fname = _updateTitleSignatory.fname;
      }

      if (_updateTitleSignatory.mname === null) {
        mname = '';
      } else {
        mname = _updateTitleSignatory.mname;
      }

      if (_updateTitleSignatory.lname === null) {
        lname = '';
      } else {
        lname = _updateTitleSignatory.lname;
      }

      if (_updateTitleSignatory.ename === null) {
        ename = '';
      } else {
        ename = _updateTitleSignatory.ename;
      }

      if (this.signator == 'Received') {
        this.formSOA.ReceivedBy = lname + ', ' + fname + ' ' + mname + ' ' + ename;
        this.formSOA.ReceivedByPos = _updateTitleSignatory.position;
      } else if (this.signator == 'Prepared') {
        this.formSOA.PreparedBy = lname + ', ' + fname + ' ' + mname + ' ' + ename;
        this.formSOA.PreparedByPos = _updateTitleSignatory.position;
      } else if (this.signator == 'Checked') {
        this.formSOA.CheckedBy = lname + ', ' + fname + ' ' + mname + ' ' + ename;
        this.formSOA.CheckedByPos = _updateTitleSignatory.position;
      } else if (this.signator == 'Noted') {
        this.formSOA.NotedBy = lname + ', ' + fname + ' ' + mname + ' ' + ename;
        this.formSOA.NotedByPos = _updateTitleSignatory.position; // console.log(updatedTitle);
      }
    },
    searchSignatoryNotedFunction: function searchSignatoryNotedFunction() {
      this.signator = 'Noted';
      $('#searchSignatory').modal('show');
    },
    searchSignatoryCheckedFunction: function searchSignatoryCheckedFunction() {
      this.signator = 'Checked';
      $('#searchSignatory').modal('show');
    },
    searchSignatoryReceivedFunction: function searchSignatoryReceivedFunction() {
      this.signator = 'Received';
      $('#searchSignatory').modal('show');
    },
    searchSignatoryPreparedFunction: function searchSignatoryPreparedFunction() {
      this.signator = 'Prepared';
      $('#searchSignatory').modal('show');
    },
    tableToExcel: function tableToExcel(table, name) {
      if (!table.nodeType) table = this.$refs.table;
      var ctx = {
        worksheet: name || 'Worksheet',
        table: table.innerHTML
      };
      window.location.href = this.uri + this.base64(this.format(this.template, ctx));
    },
    downloadPDF: function downloadPDF() {
      // var cont  = "<div align='center' text-indent='50px' style='font-size: 14px;'>  &#160;"+document.getElementById('stringinput').innerHTML +"</div>";
      //console.log(cont);
      var imageData = 'data:image/png;base64,';
      window.jsPDF = __webpack_require__(/*! jspdf */ "./node_modules/jspdf/dist/jspdf.min.js");
      var doc = new jsPDF('l', 'pt', 'legal');
      var pageHeight = 650;
      var startsY = 10;
      var totalPagesExp = '{total_pages_count_string}';
      doc.setProperties({
        title: 'PHB REPORT as of ' + this.DateFrom + ' to ' + this.DateTo + '.pdf'
      }); //HEADER

      doc.setFont('helvetica');
      doc.setFontType('bold');
      doc.setFontSize(14);
      doc.text('GENERAL SERVICES MULTI-PURPOSE COOPERATIVE', 526, 50, 'center');
      doc.setFontSize(10);
      doc.setFontType('regular');
      doc.text('Office Address: Borja Road, Damilag, Manolo Fortich, Bukidnon', 526, 60, 'center');
      doc.text('CDA # 9520-10019987-1 / TIN 411-478-949-000', 526, 70, 'center');
      doc.setFontType('bold');
      doc.text('STANDARD PHB DAILY VEHICLE USAGE REPORT', 526, 85, 'center');
      doc.setFontType('regular');
      doc.addImage(imageData, 'JPEG', 290, 40, 50, 50);
      doc.setLineWidth(1.5);
      doc.line(40, 94, 966, 94);
      doc.setLineWidth(2);
      doc.line(40, 97, 966, 97); //END HEADER

      doc.setFontSize(8);
      var datefroms = moment__WEBPACK_IMPORTED_MODULE_0___default()(this.DateFrom).format('MMMM DD, YYYY');
      var datetos = moment__WEBPACK_IMPORTED_MODULE_0___default()(this.DateTo).format('MMMM DD, YYYY');
      var datetoday = moment__WEBPACK_IMPORTED_MODULE_0___default()(this.DateToday).format('MMMM DD, YYYY');
      doc.text('DATE:', 758, 115, 'right');
      doc.setFontType('bold');
      doc.text(datetoday, 828, 115, 'right');
      doc.setFontType('regular');
      doc.text('PERIOD COVERAGE: ', 40, 115, 'left');
      doc.setFontType('bold');
      doc.text(datefroms + ' - ' + datetos, 120, 115, 'left');
      doc.autoTable({
        html: '#my-table',
        startY: 120,
        headStyles: {
          font: 'Times',
          fontSize: 10,
          cellWidth: 'wrap'
        },
        bodyStyles: {
          font: 'Times',
          fontSize: 8
        },
        didDrawPage: function didDrawPage(data) {
          var footerStr = 'Page ' + doc.internal.getNumberOfPages();

          if (typeof doc.putTotalPages === 'function') {
            footerStr = footerStr + ' of ' + totalPagesExp;
          }

          doc.setFontSize(10);
          doc.text(footerStr, data.settings.margin.left + 640, doc.internal.pageSize.height - 30, 'right');
        }
      });

      if (typeof doc.putTotalPages === 'function') {
        doc.putTotalPages(totalPagesExp);
      } //pdf.autoTable(columns, rows);
      // pdf.save('JEEP REPORT as of '+this.DateFrom+' to '+this.DateTo+'.pdf');


      var finalY = doc.lastAutoTable.finalY;
      console.log('Last Y axis sa First Table' + finalY);

      var numeral = __webpack_require__(/*! numeral */ "./node_modules/numeral/numeral.js");

      var tots = numeral(this.total).format('0,0.00');
      doc.autoTable({
        head: [['````', '````', '````', '````', '````', '````', 'TOTAL:', 'Per Day ' + tots, 'Bill Amt ' + tots]],
        // body: [
        //     ['', '','','','','','TOTAL:',tots,tots],
        // ...
        // ],
        headStyles: {
          font: 'Times',
          fontSize: 9,
          cellWidth: 'wrap'
        } //      startY: finalY

      });
      var finalYs = doc.lastAutoTable.finalY;

      if (finalYs >= pageHeight) {
        doc.addPage();
        finalYs = 30; // Restart height position
      }

      console.log('Last Get Y axis sa last nga table ' + finalYs);
      doc.setFontType('regular');
      doc.setFontSize(8);
      doc.text('Prepared By:', 40, finalYs + 20, 'left');
      doc.text(this.formSOA.PreparedBy, 90, finalYs + 30, 'left');
      var textWidths = doc.getTextWidth(this.formSOA.PreparedBy);
      doc.setLineWidth(1.5);
      doc.line(90, finalYs + 33, 90 + textWidths, finalYs + 33);
      doc.text(this.formSOA.PreparedByPos, 90, finalYs + 40, 'left'); //ACROSS Prepared

      doc.setFontType('regular');
      doc.setFontSize(8);
      doc.text('Received By:', 260, finalYs + 20, 'left');
      doc.setFontType('regular');
      doc.text(this.formSOA.ReceivedBy, 310, finalYs + 30, 'left');
      var textWidthssss = doc.getTextWidth(this.formSOA.ReceivedBy);
      doc.setLineWidth(1.5);
      doc.line(310, finalYs + 33, 310 + textWidthssss, finalYs + 33);
      doc.text(this.formSOA.ReceivedByPos, 310, finalYs + 40, 'left'); //END ACROSS

      doc.setFontType('regular');
      doc.setFontSize(8);
      doc.text('Checked by:', 40, finalYs + 60, 'left');
      doc.text(this.formSOA.CheckedBy, 90, finalYs + 70, 'left');
      var textWidthss = doc.getTextWidth(this.formSOA.CheckedBy);
      doc.setLineWidth(1.5);
      doc.line(90, finalYs + 73, 90 + textWidthss, finalYs + 73);
      doc.text(this.formSOA.CheckedByPos, 90, finalYs + 80, 'left');
      doc.setFontType('regular');
      doc.setFontSize(8);
      doc.text('Noted by:', 40, finalYs + 100, 'left');
      doc.text(this.formSOA.NotedBy, 90, finalYs + 110, 'left');
      var textWidthsss = doc.getTextWidth(this.formSOA.NotedBy);
      doc.setLineWidth(1.5);
      doc.line(90, finalYs + 113, 90 + textWidthsss, finalYs + 113);
      doc.text(this.formSOA.NotedByPos, 90, finalYs + 120, 'left'); //doc.text(15, finalY+10,tots);

      window.open(doc.output('bloburl'), '_blank');
    },
    loadJVL: function loadJVL() {
      var _this = this;

      axios.get('api/getphbvl').then(function (response) {
        response.data.forEach(function (item) {
          if (!item.Collected) {
            item.Collected = 0;
          }

          item.Balance = item.BillAmount - item.Collected;

          if (item.Balance > 0) {
            item.CalculatedStatus = 'UNPAID';
          } else if (item.Balance == 0) {
            item.CalculatedStatus = 'PAID';
          } else {
            item.CalculatedStatus = 'ERR';
          }
        });
        _this.jvls = response.data;
      })["catch"](function (error) {
        console.log(error);
      });
    }
  },
  created: function created() {
    this.loadJVL();
    var d = new Date();
    var today = new Date().toISOString().slice(0, 10);
    this.DateFrom = today;
    this.DateTo = today;
    this.DateToday = today;
  },
  computed: {
    filteredBlogs: function filteredBlogs() {
      var vm = this;
      var startdate = vm.DateFrom;
      var enddate = vm.DateTo;
      return _.filter(vm.jvls, function (data) {
        if (_.isNull(startdate) && _.isNull(enddate)) {
          return true;
        } else {
          var date = data.PHBVLDate;
          var cons = date >= startdate && date <= enddate;
          return cons;
          /* vm.sum = vm.jvls.filter((item) =>item.BillAmount)
                                 .map((item) => +item.BillAmount)
                                 .reduce((sum, current) => sum + current);
                             console.log(vm.sum);
                          return vm.sum;*/
        }
      });
      /*else{
                   this.DateFrom = "";
                   this.DateTo = ""
                   //return this.jvls.filter(jvl =>{
                   //return jvl.JeepPlateNo.includes(this.form.JeepPlateNo)
                 //});
                 return this.jvls.filter((samsung) => {
                   return this.SearchJeepPlateNo.toLowerCase().split(' ').every(v => samsung.JeepPlateNo.toLowerCase().includes(v));
               });
                 }*/
    },
    total: function total() {
      var vm = this;
      /*vm.sum = vm.filteredBlogs.filter((item) =>item.BillAmount)
                               .map((item) => +item.BillAmount)
                               .reduce((sum, current) => sum + current);
                           console.log(vm.sum);
                        return vm.sum;*/

      return vm.filteredBlogs.reduce(function (sum, item) {
        var tot = sum + item.BillAmount;
        vm.sum = tot;
        return sum + item.BillAmount;
      }, 0);
    }
  }
});

/***/ }),

/***/ "./node_modules/css-loader/index.js?!./node_modules/vue-loader/lib/loaders/stylePostLoader.js!./node_modules/postcss-loader/src/index.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=style&index=0&lang=css&":
/*!*********************************************************************************************************************************************************************************************************************************************************************************************!*\
  !*** ./node_modules/css-loader??ref--6-1!./node_modules/vue-loader/lib/loaders/stylePostLoader.js!./node_modules/postcss-loader/src??ref--6-2!./node_modules/vue-loader/lib??vue-loader-options!./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=style&index=0&lang=css& ***!
  \*********************************************************************************************************************************************************************************************************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {

exports = module.exports = __webpack_require__(/*! ../../../../node_modules/css-loader/lib/css-base.js */ "./node_modules/css-loader/lib/css-base.js")(false);
// imports


// module
exports.push([module.i, "\ninput[data-readonly] {\r\n\tpointer-events: none;\n}\r\n", ""]);

// exports


/***/ }),

/***/ "./node_modules/style-loader/index.js!./node_modules/css-loader/index.js?!./node_modules/vue-loader/lib/loaders/stylePostLoader.js!./node_modules/postcss-loader/src/index.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=style&index=0&lang=css&":
/*!*************************************************************************************************************************************************************************************************************************************************************************************************************************!*\
  !*** ./node_modules/style-loader!./node_modules/css-loader??ref--6-1!./node_modules/vue-loader/lib/loaders/stylePostLoader.js!./node_modules/postcss-loader/src??ref--6-2!./node_modules/vue-loader/lib??vue-loader-options!./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=style&index=0&lang=css& ***!
  \*************************************************************************************************************************************************************************************************************************************************************************************************************************/
/*! no static exports found */
/***/ (function(module, exports, __webpack_require__) {


var content = __webpack_require__(/*! !../../../../node_modules/css-loader??ref--6-1!../../../../node_modules/vue-loader/lib/loaders/stylePostLoader.js!../../../../node_modules/postcss-loader/src??ref--6-2!../../../../node_modules/vue-loader/lib??vue-loader-options!./PHBReportList.vue?vue&type=style&index=0&lang=css& */ "./node_modules/css-loader/index.js?!./node_modules/vue-loader/lib/loaders/stylePostLoader.js!./node_modules/postcss-loader/src/index.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=style&index=0&lang=css&");

if(typeof content === 'string') content = [[module.i, content, '']];

var transform;
var insertInto;



var options = {"hmr":true}

options.transform = transform
options.insertInto = undefined;

var update = __webpack_require__(/*! ../../../../node_modules/style-loader/lib/addStyles.js */ "./node_modules/style-loader/lib/addStyles.js")(content, options);

if(content.locals) module.exports = content.locals;

if(false) {}

/***/ }),

/***/ "./node_modules/vue-loader/lib/loaders/templateLoader.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=template&id=26dabb92&":
/*!******************************************************************************************************************************************************************************************************************************!*\
  !*** ./node_modules/vue-loader/lib/loaders/templateLoader.js??vue-loader-options!./node_modules/vue-loader/lib??vue-loader-options!./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=template&id=26dabb92& ***!
  \******************************************************************************************************************************************************************************************************************************/
/*! exports provided: render, staticRenderFns */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "render", function() { return render; });
/* harmony export (binding) */ __webpack_require__.d(__webpack_exports__, "staticRenderFns", function() { return staticRenderFns; });
var render = function() {
  var _vm = this
  var _h = _vm.$createElement
  var _c = _vm._self._c || _h
  return _c(
    "div",
    { staticClass: "container" },
    [
      _c(
        "nav",
        {
          staticClass:
            "navbar navbar-dark bg-dark navbar-expand-lg navbar-light bg-light"
        },
        [
          _c(
            "div",
            {
              staticClass: "collapse navbar-collapse",
              attrs: { id: "navbarNavDropdown" }
            },
            [
              _c("ul", { staticClass: "navbar-nav" }, [
                _c("li", { staticClass: "nav-item dropdown" }, [
                  _c(
                    "a",
                    {
                      staticClass: "nav-link dropdown-toggle",
                      attrs: {
                        href: "#",
                        id: "navbarDropdownMenuLink",
                        role: "button",
                        "data-toggle": "dropdown",
                        "aria-haspopup": "true",
                        "aria-expanded": "false",
                        "data-target": "#mlist"
                      }
                    },
                    [
                      _vm._v(
                        "\n                              Master File\n                              "
                      )
                    ]
                  ),
                  _vm._v(" "),
                  _c(
                    "ul",
                    {
                      staticClass: "dropdown-menu",
                      attrs: {
                        "aria-labelledby": "navbarDropdownMenuLink",
                        id: "mlist"
                      }
                    },
                    [
                      _c(
                        "li",
                        [
                          _c(
                            "router-link",
                            {
                              staticClass: "dropdown-item",
                              attrs: { to: "/driverlist" }
                            },
                            [_c("a", [_vm._v("Driver List")])]
                          )
                        ],
                        1
                      ),
                      _vm._v(" "),
                      _c(
                        "li",
                        [
                          _c(
                            "router-link",
                            {
                              staticClass: "dropdown-item",
                              attrs: { to: "/phbvehiclelist" }
                            },
                            [_c("a", [_vm._v("PHB Vehicle List")])]
                          )
                        ],
                        1
                      ),
                      _vm._v(" "),
                      _c(
                        "li",
                        [
                          _c(
                            "router-link",
                            {
                              staticClass: "dropdown-item",
                              attrs: { to: "/vehicletype" }
                            },
                            [_c("a", [_vm._v("Vehicle Type List")])]
                          )
                        ],
                        1
                      ),
                      _vm._v(" "),
                      _c(
                        "li",
                        [
                          _c(
                            "router-link",
                            {
                              staticClass: "dropdown-item",
                              attrs: { to: "/phbvehiclerate" }
                            },
                            [_c("a", [_vm._v("PHB Rate List")])]
                          )
                        ],
                        1
                      ),
                      _vm._v(" "),
                      _c(
                        "li",
                        [
                          _c(
                            "router-link",
                            {
                              staticClass: "dropdown-item",
                              attrs: { to: "/clientlist" }
                            },
                            [_c("a", [_vm._v("Client List")])]
                          )
                        ],
                        1
                      ),
                      _vm._v(" "),
                      _c(
                        "li",
                        [
                          _c(
                            "router-link",
                            {
                              staticClass: "dropdown-item",
                              attrs: { to: "/locationlist" }
                            },
                            [_c("a", [_vm._v("Location List")])]
                          )
                        ],
                        1
                      )
                    ]
                  )
                ]),
                _vm._v(" "),
                _c("li", { staticClass: "nav-item dropdown" }, [
                  _c(
                    "a",
                    {
                      staticClass: "nav-link dropdown-toggle",
                      attrs: {
                        href: "#",
                        id: "navbarDropdownMenuLink",
                        role: "button",
                        "data-toggle": "dropdown",
                        "aria-haspopup": "true",
                        "aria-expanded": "false",
                        "data-target": "#trans"
                      }
                    },
                    [
                      _vm._v(
                        "\n                              Transactions\n                              "
                      )
                    ]
                  ),
                  _vm._v(" "),
                  _c(
                    "ul",
                    {
                      staticClass: "dropdown-menu",
                      attrs: {
                        "aria-labelledby": "navbarDropdownMenuLink",
                        id: "trans"
                      }
                    },
                    [
                      _c(
                        "li",
                        [
                          _c(
                            "router-link",
                            {
                              staticClass: "dropdown-item",
                              attrs: { to: "/phbvehiclelogentry" }
                            },
                            [_c("a", [_vm._v("PHB Vehicle Log Entry")])]
                          )
                        ],
                        1
                      )
                    ]
                  )
                ]),
                _vm._v(" "),
                _c("li", { staticClass: "nav-item dropdown" }, [
                  _c(
                    "a",
                    {
                      staticClass: "nav-link dropdown-toggle",
                      attrs: {
                        href: "#",
                        id: "navbarDropdownMenuLink",
                        role: "button",
                        "data-toggle": "dropdown",
                        "aria-haspopup": "true",
                        "aria-expanded": "false",
                        "data-target": "#report"
                      }
                    },
                    [
                      _vm._v(
                        "\n                              PHB Reports\n                              "
                      )
                    ]
                  ),
                  _vm._v(" "),
                  _c(
                    "ul",
                    {
                      staticClass: "dropdown-menu",
                      attrs: {
                        "aria-labelledby": "navbarDropdownMenuLink",
                        id: "report"
                      }
                    },
                    [
                      _c(
                        "li",
                        [
                          _c(
                            "router-link",
                            {
                              staticClass: "dropdown-item",
                              attrs: { to: "/reportlistPHB" }
                            },
                            [_c("a", [_vm._v("Standard PHB Report")])]
                          )
                        ],
                        1
                      ),
                      _vm._v(" "),
                      _c(
                        "li",
                        [
                          _c(
                            "router-link",
                            {
                              staticClass: "dropdown-item",
                              attrs: { to: "/reportlistPHBPerJeep" }
                            },
                            [
                              _c("a", [
                                _vm._v("PHB's Vehicle Log Billing Report")
                              ])
                            ]
                          )
                        ],
                        1
                      )
                    ]
                  )
                ])
              ])
            ]
          )
        ]
      ),
      _vm._v(" "),
      _c("div", { staticClass: "col-xs-12" }, [
        _c("div", { staticClass: "card" }, [
          _c("div", { staticClass: "card-header" }, [
            _vm._m(0),
            _vm._v(" "),
            _c("div", { staticClass: "card-tools" }),
            _vm._v(" "),
            _c("br"),
            _vm._v(" "),
            _c("br"),
            _vm._v(" "),
            _c("div", { staticClass: "row" }, [
              _c("div", { staticClass: "col" }, [
                _c("div", { staticClass: "input-group mb-3 input-group-sm" }, [
                  _vm._m(1),
                  _vm._v(" "),
                  _c("input", {
                    directives: [
                      {
                        name: "model",
                        rawName: "v-model",
                        value: _vm.DateFrom,
                        expression: "DateFrom"
                      }
                    ],
                    staticClass: "form-control",
                    attrs: { id: "DateFrom", type: "date", name: "DateFrom" },
                    domProps: { value: _vm.DateFrom },
                    on: {
                      input: function($event) {
                        if ($event.target.composing) {
                          return
                        }
                        _vm.DateFrom = $event.target.value
                      }
                    }
                  })
                ])
              ]),
              _vm._v(" "),
              _c("div", { staticClass: "col" }, [
                _c("div", { staticClass: "input-group mb-3 input-group-sm" }, [
                  _vm._m(2),
                  _vm._v(" "),
                  _c("input", {
                    directives: [
                      {
                        name: "model",
                        rawName: "v-model",
                        value: _vm.DateTo,
                        expression: "DateTo"
                      }
                    ],
                    staticClass: "form-control",
                    attrs: { id: "DateTo", type: "date", name: "DateTo" },
                    domProps: { value: _vm.DateTo },
                    on: {
                      input: function($event) {
                        if ($event.target.composing) {
                          return
                        }
                        _vm.DateTo = $event.target.value
                      }
                    }
                  })
                ])
              ])
            ]),
            _vm._v(" "),
            _c("div", { staticClass: "row" }, [
              _c("div", { staticClass: "col" }, [
                _c(
                  "div",
                  { staticClass: "input-group mb-3 input-group-sm" },
                  [
                    _vm._m(3),
                    _vm._v(" "),
                    _c("input", {
                      directives: [
                        {
                          name: "model",
                          rawName: "v-model",
                          value: _vm.formSOA.PreparedBy,
                          expression: "formSOA.PreparedBy"
                        }
                      ],
                      staticClass: "form-control",
                      attrs: { type: "text", name: "PreparedBy", readonly: "" },
                      domProps: { value: _vm.formSOA.PreparedBy },
                      on: {
                        input: function($event) {
                          if ($event.target.composing) {
                            return
                          }
                          _vm.$set(
                            _vm.formSOA,
                            "PreparedBy",
                            $event.target.value
                          )
                        }
                      }
                    }),
                    _vm._v(" "),
                    _c(
                      "b-input-group-append",
                      [
                        _c(
                          "b-button",
                          {
                            attrs: { variant: "outline-primary", size: "sm" },
                            on: {
                              click: function($event) {
                                return _vm.searchSignatoryPreparedFunction()
                              }
                            }
                          },
                          [
                            _c("i", {
                              staticClass: "fa fa-search",
                              attrs: { "aria-hidden": "true" }
                            })
                          ]
                        )
                      ],
                      1
                    )
                  ],
                  1
                )
              ]),
              _vm._v(" "),
              _c("div", { staticClass: "col" }, [
                _c(
                  "div",
                  { staticClass: "input-group mb-3 input-group-sm" },
                  [
                    _vm._m(4),
                    _vm._v(" "),
                    _c("input", {
                      directives: [
                        {
                          name: "model",
                          rawName: "v-model",
                          value: _vm.formSOA.ReceivedBy,
                          expression: "formSOA.ReceivedBy"
                        }
                      ],
                      staticClass: "form-control",
                      attrs: { type: "text", name: "ReceivedBy", readonly: "" },
                      domProps: { value: _vm.formSOA.ReceivedBy },
                      on: {
                        input: function($event) {
                          if ($event.target.composing) {
                            return
                          }
                          _vm.$set(
                            _vm.formSOA,
                            "ReceivedBy",
                            $event.target.value
                          )
                        }
                      }
                    }),
                    _vm._v(" "),
                    _c(
                      "b-input-group-append",
                      [
                        _c(
                          "b-button",
                          {
                            attrs: { variant: "outline-primary", size: "sm" },
                            on: {
                              click: function($event) {
                                return _vm.searchSignatoryReceivedFunction()
                              }
                            }
                          },
                          [
                            _c("i", {
                              staticClass: "fa fa-search",
                              attrs: { "aria-hidden": "true" }
                            })
                          ]
                        )
                      ],
                      1
                    )
                  ],
                  1
                )
              ]),
              _vm._v(" "),
              _c("div", { staticClass: "col" }, [
                _c(
                  "div",
                  { staticClass: "input-group mb-3 input-group-sm" },
                  [
                    _vm._m(5),
                    _vm._v(" "),
                    _c("input", {
                      directives: [
                        {
                          name: "model",
                          rawName: "v-model",
                          value: _vm.formSOA.CheckedBy,
                          expression: "formSOA.CheckedBy"
                        }
                      ],
                      staticClass: "form-control",
                      attrs: { type: "text", name: "CheckedBy", readonly: "" },
                      domProps: { value: _vm.formSOA.CheckedBy },
                      on: {
                        input: function($event) {
                          if ($event.target.composing) {
                            return
                          }
                          _vm.$set(
                            _vm.formSOA,
                            "CheckedBy",
                            $event.target.value
                          )
                        }
                      }
                    }),
                    _vm._v(" "),
                    _c(
                      "b-input-group-append",
                      [
                        _c(
                          "b-button",
                          {
                            attrs: { variant: "outline-primary", size: "sm" },
                            on: {
                              click: function($event) {
                                return _vm.searchSignatoryCheckedFunction()
                              }
                            }
                          },
                          [
                            _c("i", {
                              staticClass: "fa fa-search",
                              attrs: { "aria-hidden": "true" }
                            })
                          ]
                        )
                      ],
                      1
                    )
                  ],
                  1
                )
              ]),
              _vm._v(" "),
              _c("div", { staticClass: "col" }, [
                _c(
                  "div",
                  { staticClass: "input-group mb-3 input-group-sm" },
                  [
                    _vm._m(6),
                    _vm._v(" "),
                    _c("input", {
                      directives: [
                        {
                          name: "model",
                          rawName: "v-model",
                          value: _vm.formSOA.NotedBy,
                          expression: "formSOA.NotedBy"
                        }
                      ],
                      staticClass: "form-control",
                      attrs: { type: "text", name: "NotedBy", readonly: "" },
                      domProps: { value: _vm.formSOA.NotedBy },
                      on: {
                        input: function($event) {
                          if ($event.target.composing) {
                            return
                          }
                          _vm.$set(_vm.formSOA, "NotedBy", $event.target.value)
                        }
                      }
                    }),
                    _vm._v(" "),
                    _c(
                      "b-input-group-append",
                      [
                        _c(
                          "b-button",
                          {
                            attrs: { variant: "outline-primary", size: "sm" },
                            on: {
                              click: function($event) {
                                return _vm.searchSignatoryNotedFunction()
                              }
                            }
                          },
                          [
                            _c("i", {
                              staticClass: "fa fa-search",
                              attrs: { "aria-hidden": "true" }
                            })
                          ]
                        )
                      ],
                      1
                    )
                  ],
                  1
                )
              ])
            ]),
            _vm._v(" "),
            _c("div", { staticClass: "row" }, [
              _c("div", { staticClass: "col" }, [
                _c("div", { staticClass: "input-group mb-3 input-group-sm" }, [
                  _vm._m(7),
                  _vm._v(" "),
                  _c("input", {
                    directives: [
                      {
                        name: "model",
                        rawName: "v-model",
                        value: _vm.formSOA.PreparedByPos,
                        expression: "formSOA.PreparedByPos"
                      }
                    ],
                    staticClass: "form-control",
                    attrs: {
                      type: "text",
                      name: "PreparedByPos",
                      readonly: ""
                    },
                    domProps: { value: _vm.formSOA.PreparedByPos },
                    on: {
                      input: function($event) {
                        if ($event.target.composing) {
                          return
                        }
                        _vm.$set(
                          _vm.formSOA,
                          "PreparedByPos",
                          $event.target.value
                        )
                      }
                    }
                  })
                ])
              ]),
              _vm._v(" "),
              _c("div", { staticClass: "col" }, [
                _c("div", { staticClass: "input-group mb-3 input-group-sm" }, [
                  _c(
                    "div",
                    { staticClass: "input-group mb-3 input-group-sm" },
                    [
                      _vm._m(8),
                      _vm._v(" "),
                      _c("input", {
                        directives: [
                          {
                            name: "model",
                            rawName: "v-model",
                            value: _vm.formSOA.ReceivedByPos,
                            expression: "formSOA.ReceivedByPos"
                          }
                        ],
                        staticClass: "form-control",
                        attrs: {
                          type: "text",
                          name: "ReceivedByPos",
                          readonly: ""
                        },
                        domProps: { value: _vm.formSOA.ReceivedByPos },
                        on: {
                          input: function($event) {
                            if ($event.target.composing) {
                              return
                            }
                            _vm.$set(
                              _vm.formSOA,
                              "ReceivedByPos",
                              $event.target.value
                            )
                          }
                        }
                      })
                    ]
                  )
                ])
              ]),
              _vm._v(" "),
              _c("div", { staticClass: "col" }, [
                _c("div", { staticClass: "input-group mb-3 input-group-sm" }, [
                  _c(
                    "div",
                    { staticClass: "input-group mb-3 input-group-sm" },
                    [
                      _vm._m(9),
                      _vm._v(" "),
                      _c("input", {
                        directives: [
                          {
                            name: "model",
                            rawName: "v-model",
                            value: _vm.formSOA.CheckedByPos,
                            expression: "formSOA.CheckedByPos"
                          }
                        ],
                        staticClass: "form-control",
                        attrs: {
                          type: "text",
                          name: "CheckedByPos",
                          readonly: ""
                        },
                        domProps: { value: _vm.formSOA.CheckedByPos },
                        on: {
                          input: function($event) {
                            if ($event.target.composing) {
                              return
                            }
                            _vm.$set(
                              _vm.formSOA,
                              "CheckedByPos",
                              $event.target.value
                            )
                          }
                        }
                      })
                    ]
                  )
                ])
              ]),
              _vm._v(" "),
              _c("div", { staticClass: "col" }, [
                _c("div", { staticClass: "input-group mb-3 input-group-sm" }, [
                  _vm._m(10),
                  _vm._v(" "),
                  _c("input", {
                    directives: [
                      {
                        name: "model",
                        rawName: "v-model",
                        value: _vm.formSOA.NotedByPos,
                        expression: "formSOA.NotedByPos"
                      }
                    ],
                    staticClass: "form-control",
                    attrs: { type: "text", name: "NotedByPos", readonly: "" },
                    domProps: { value: _vm.formSOA.NotedByPos },
                    on: {
                      input: function($event) {
                        if ($event.target.composing) {
                          return
                        }
                        _vm.$set(_vm.formSOA, "NotedByPos", $event.target.value)
                      }
                    }
                  })
                ])
              ])
            ])
          ]),
          _vm._v(" "),
          _c(
            "div",
            {
              ref: "table",
              staticClass: "card-body table-responsive pre-scrollable"
            },
            [
              _c(
                "table",
                { staticClass: "table table-hover", attrs: { id: "my-table" } },
                [
                  _vm._m(11),
                  _vm._v(" "),
                  _c(
                    "tbody",
                    _vm._l(_vm.filteredBlogs, function(jvl) {
                      return _c("tr", { key: jvl.PHBVLHDRID }, [
                        _c("td", [_vm._v(_vm._s(jvl.PHBVLDate))]),
                        _vm._v(" "),
                        _c("td", [_vm._v(_vm._s(jvl.OVLNo))]),
                        _vm._v(" "),
                        _c("td", [_vm._v(_vm._s(jvl.DriverName))]),
                        _vm._v(" "),
                        _c("td", [_vm._v(_vm._s(jvl.PHBPlateNo))]),
                        _vm._v(" "),
                        _c("td", [_vm._v(_vm._s(jvl.LocationName))]),
                        _vm._v(" "),
                        _c("td", [_vm._v(_vm._s(jvl.GLCode))]),
                        _vm._v(" "),
                        _c("td", [_vm._v(_vm._s(jvl.CostCenter))]),
                        _vm._v(" "),
                        _c("td", [_vm._v(_vm._s(jvl.TimeIn))]),
                        _vm._v(" "),
                        _c("td", [_vm._v(_vm._s(jvl.TimeOut))]),
                        _vm._v(" "),
                        _c("td", [_vm._v(_vm._s(jvl.startreading))]),
                        _vm._v(" "),
                        _c("td", [_vm._v(_vm._s(jvl.endreading))]),
                        _vm._v(" "),
                        _c("td", [_vm._v(_vm._s(jvl.totalrun))]),
                        _vm._v(" "),
                        _c("td", [_vm._v(_vm._s(jvl.actualstartreading))]),
                        _vm._v(" "),
                        _c("td", [_vm._v(_vm._s(jvl.actualendreading))]),
                        _vm._v(" "),
                        _c("td", [_vm._v(_vm._s(jvl.totalactualrun))]),
                        _vm._v(" "),
                        _c("td", { staticClass: "text-center" }, [
                          _vm._v(_vm._s(jvl.PerKilometerRate.toFixed(2)))
                        ]),
                        _vm._v(" "),
                        _c("td", { staticClass: "text-right" }, [
                          _vm._v(_vm._s(jvl.BillAmount.toFixed(2)))
                        ]),
                        _vm._v(" "),
                        _c("td", { staticClass: "text-right" }, [
                          _vm._v(_vm._s(jvl.Collected.toFixed(2)))
                        ]),
                        _vm._v(" "),
                        _c("td", { staticClass: "text-right" }, [
                          _vm._v(_vm._s(jvl.Balance.toFixed(2)))
                        ]),
                        _vm._v(" "),
                        _c("td", [_vm._v(_vm._s(jvl.CalculatedStatus))])
                      ])
                    }),
                    0
                  )
                ]
              )
            ]
          ),
          _vm._v(" "),
          _c("div", { staticClass: "card-footer" }, [
            _c(
              "button",
              {
                staticClass: "btn btn-warning",
                on: { click: _vm.downloadPDF }
              },
              [
                _vm._v("\n            Print Report\n            "),
                _c("i", { staticClass: "fa fa-print fa fw" })
              ]
            ),
            _vm._v(" "),
            _c(
              "button",
              {
                staticClass: "btn btn-success",
                on: {
                  click: function($event) {
                    return _vm.tableToExcel("table", "Lorem Table")
                  }
                }
              },
              [
                _vm._v("Export to Excel "),
                _c("i", { staticClass: "fa fa-file-export fa fw" })
              ]
            ),
            _vm._v(" "),
            _c("div", { staticStyle: { float: "right" } }, [
              _c("div", { staticClass: "input-group mb-3 input-group-sm" }, [
                _vm._m(12),
                _vm._v(" "),
                _c("input", {
                  staticClass: "form-control",
                  attrs: { type: "text" },
                  domProps: { value: _vm._f("numeral")(_vm.total, "0,0.00") }
                })
              ])
            ])
          ])
        ])
      ]),
      _vm._v(" "),
      _c("search-signatorys", {
        on: {
          changeTitleSignatory: function($event) {
            return _vm.updateTitleSignatory($event)
          }
        }
      })
    ],
    1
  )
}
var staticRenderFns = [
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("h3", { staticClass: "card-title" }, [
      _c("b", [_vm._v("PHB Report List")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [
        _vm._v("Search Date From")
      ])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [
        _vm._v("Search Date To")
      ])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Prepared By:")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Received By:")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Checked By:")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Noted By:")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Prepared Pos.")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Received Pos.")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Checked Pos.")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [_vm._v("Noted Pos.")])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("thead", [
      _c("tr", [
        _c("th", [_vm._v("DOC"), _c("br"), _vm._v("DATE")]),
        _vm._v(" "),
        _c("th", [_vm._v("OVL"), _c("br"), _vm._v("NO")]),
        _vm._v(" "),
        _c("th", [_vm._v("DRIVER")]),
        _vm._v(" "),
        _c("th", [_vm._v("PHB#")]),
        _vm._v(" "),
        _c("th", [_vm._v("LOCATION")]),
        _vm._v(" "),
        _c("th", [_vm._v("GL"), _c("br"), _vm._v("CODE")]),
        _vm._v(" "),
        _c("th", [_vm._v("COST"), _c("br"), _vm._v("CENTER")]),
        _vm._v(" "),
        _c("th", [_vm._v("TIME"), _c("br"), _vm._v("IN")]),
        _vm._v(" "),
        _c("th", [_vm._v("TIME"), _c("br"), _vm._v("OUT")]),
        _vm._v(" "),
        _c("th", [_vm._v("DMPI"), _c("br"), _vm._v("START")]),
        _vm._v(" "),
        _c("th", [_vm._v("DMPI"), _c("br"), _vm._v("END")]),
        _vm._v(" "),
        _c("th", [_vm._v("DMPI"), _c("br"), _vm._v("RUN")]),
        _vm._v(" "),
        _c("th", [_vm._v("ACTUAL"), _c("br"), _vm._v("START")]),
        _vm._v(" "),
        _c("th", [_vm._v("ACTUAL"), _c("br"), _vm._v("END")]),
        _vm._v(" "),
        _c("th", [_vm._v("ACTUAL"), _c("br"), _vm._v("RUN")]),
        _vm._v(" "),
        _c("th", [_vm._v("PER"), _c("br"), _vm._v("KM")]),
        _vm._v(" "),
        _c("th", [_vm._v("BILLED"), _c("br"), _vm._v("AMT")]),
        _vm._v(" "),
        _c("th", [_vm._v("COLLECT"), _c("br"), _vm._v("AMT")]),
        _vm._v(" "),
        _c("th", [_vm._v("BALANCE")]),
        _vm._v(" "),
        _c("th", [_vm._v("STATUS")])
      ])
    ])
  },
  function() {
    var _vm = this
    var _h = _vm.$createElement
    var _c = _vm._self._c || _h
    return _c("div", { staticClass: "input-group-prepend" }, [
      _c("span", { staticClass: "input-group-text" }, [
        _vm._v("Total Bill Amount")
      ])
    ])
  }
]
render._withStripped = true



/***/ }),

/***/ "./resources/js/components/PHBComponents/PHBReportList.vue":
/*!*****************************************************************!*\
  !*** ./resources/js/components/PHBComponents/PHBReportList.vue ***!
  \*****************************************************************/
/*! exports provided: default */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _PHBReportList_vue_vue_type_template_id_26dabb92___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! ./PHBReportList.vue?vue&type=template&id=26dabb92& */ "./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=template&id=26dabb92&");
/* harmony import */ var _PHBReportList_vue_vue_type_script_lang_js___WEBPACK_IMPORTED_MODULE_1__ = __webpack_require__(/*! ./PHBReportList.vue?vue&type=script&lang=js& */ "./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=script&lang=js&");
/* empty/unused harmony star reexport *//* harmony import */ var _PHBReportList_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_2__ = __webpack_require__(/*! ./PHBReportList.vue?vue&type=style&index=0&lang=css& */ "./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=style&index=0&lang=css&");
/* harmony import */ var _node_modules_vue_loader_lib_runtime_componentNormalizer_js__WEBPACK_IMPORTED_MODULE_3__ = __webpack_require__(/*! ../../../../node_modules/vue-loader/lib/runtime/componentNormalizer.js */ "./node_modules/vue-loader/lib/runtime/componentNormalizer.js");






/* normalize component */

var component = Object(_node_modules_vue_loader_lib_runtime_componentNormalizer_js__WEBPACK_IMPORTED_MODULE_3__["default"])(
  _PHBReportList_vue_vue_type_script_lang_js___WEBPACK_IMPORTED_MODULE_1__["default"],
  _PHBReportList_vue_vue_type_template_id_26dabb92___WEBPACK_IMPORTED_MODULE_0__["render"],
  _PHBReportList_vue_vue_type_template_id_26dabb92___WEBPACK_IMPORTED_MODULE_0__["staticRenderFns"],
  false,
  null,
  null,
  null
  
)

/* hot reload */
if (false) { var api; }
component.options.__file = "resources/js/components/PHBComponents/PHBReportList.vue"
/* harmony default export */ __webpack_exports__["default"] = (component.exports);

/***/ }),

/***/ "./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=script&lang=js&":
/*!******************************************************************************************!*\
  !*** ./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=script&lang=js& ***!
  \******************************************************************************************/
/*! exports provided: default */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _node_modules_babel_loader_lib_index_js_ref_4_0_node_modules_vue_loader_lib_index_js_vue_loader_options_PHBReportList_vue_vue_type_script_lang_js___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! -!../../../../node_modules/babel-loader/lib??ref--4-0!../../../../node_modules/vue-loader/lib??vue-loader-options!./PHBReportList.vue?vue&type=script&lang=js& */ "./node_modules/babel-loader/lib/index.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=script&lang=js&");
/* empty/unused harmony star reexport */ /* harmony default export */ __webpack_exports__["default"] = (_node_modules_babel_loader_lib_index_js_ref_4_0_node_modules_vue_loader_lib_index_js_vue_loader_options_PHBReportList_vue_vue_type_script_lang_js___WEBPACK_IMPORTED_MODULE_0__["default"]); 

/***/ }),

/***/ "./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=style&index=0&lang=css&":
/*!**************************************************************************************************!*\
  !*** ./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=style&index=0&lang=css& ***!
  \**************************************************************************************************/
/*! no static exports found */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _node_modules_style_loader_index_js_node_modules_css_loader_index_js_ref_6_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_6_2_node_modules_vue_loader_lib_index_js_vue_loader_options_PHBReportList_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! -!../../../../node_modules/style-loader!../../../../node_modules/css-loader??ref--6-1!../../../../node_modules/vue-loader/lib/loaders/stylePostLoader.js!../../../../node_modules/postcss-loader/src??ref--6-2!../../../../node_modules/vue-loader/lib??vue-loader-options!./PHBReportList.vue?vue&type=style&index=0&lang=css& */ "./node_modules/style-loader/index.js!./node_modules/css-loader/index.js?!./node_modules/vue-loader/lib/loaders/stylePostLoader.js!./node_modules/postcss-loader/src/index.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=style&index=0&lang=css&");
/* harmony import */ var _node_modules_style_loader_index_js_node_modules_css_loader_index_js_ref_6_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_6_2_node_modules_vue_loader_lib_index_js_vue_loader_options_PHBReportList_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_0___default = /*#__PURE__*/__webpack_require__.n(_node_modules_style_loader_index_js_node_modules_css_loader_index_js_ref_6_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_6_2_node_modules_vue_loader_lib_index_js_vue_loader_options_PHBReportList_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_0__);
/* harmony reexport (unknown) */ for(var __WEBPACK_IMPORT_KEY__ in _node_modules_style_loader_index_js_node_modules_css_loader_index_js_ref_6_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_6_2_node_modules_vue_loader_lib_index_js_vue_loader_options_PHBReportList_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_0__) if(__WEBPACK_IMPORT_KEY__ !== 'default') (function(key) { __webpack_require__.d(__webpack_exports__, key, function() { return _node_modules_style_loader_index_js_node_modules_css_loader_index_js_ref_6_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_6_2_node_modules_vue_loader_lib_index_js_vue_loader_options_PHBReportList_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_0__[key]; }) }(__WEBPACK_IMPORT_KEY__));
 /* harmony default export */ __webpack_exports__["default"] = (_node_modules_style_loader_index_js_node_modules_css_loader_index_js_ref_6_1_node_modules_vue_loader_lib_loaders_stylePostLoader_js_node_modules_postcss_loader_src_index_js_ref_6_2_node_modules_vue_loader_lib_index_js_vue_loader_options_PHBReportList_vue_vue_type_style_index_0_lang_css___WEBPACK_IMPORTED_MODULE_0___default.a); 

/***/ }),

/***/ "./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=template&id=26dabb92&":
/*!************************************************************************************************!*\
  !*** ./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=template&id=26dabb92& ***!
  \************************************************************************************************/
/*! exports provided: render, staticRenderFns */
/***/ (function(module, __webpack_exports__, __webpack_require__) {

"use strict";
__webpack_require__.r(__webpack_exports__);
/* harmony import */ var _node_modules_vue_loader_lib_loaders_templateLoader_js_vue_loader_options_node_modules_vue_loader_lib_index_js_vue_loader_options_PHBReportList_vue_vue_type_template_id_26dabb92___WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(/*! -!../../../../node_modules/vue-loader/lib/loaders/templateLoader.js??vue-loader-options!../../../../node_modules/vue-loader/lib??vue-loader-options!./PHBReportList.vue?vue&type=template&id=26dabb92& */ "./node_modules/vue-loader/lib/loaders/templateLoader.js?!./node_modules/vue-loader/lib/index.js?!./resources/js/components/PHBComponents/PHBReportList.vue?vue&type=template&id=26dabb92&");
/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "render", function() { return _node_modules_vue_loader_lib_loaders_templateLoader_js_vue_loader_options_node_modules_vue_loader_lib_index_js_vue_loader_options_PHBReportList_vue_vue_type_template_id_26dabb92___WEBPACK_IMPORTED_MODULE_0__["render"]; });

/* harmony reexport (safe) */ __webpack_require__.d(__webpack_exports__, "staticRenderFns", function() { return _node_modules_vue_loader_lib_loaders_templateLoader_js_vue_loader_options_node_modules_vue_loader_lib_index_js_vue_loader_options_PHBReportList_vue_vue_type_template_id_26dabb92___WEBPACK_IMPORTED_MODULE_0__["staticRenderFns"]; });



/***/ })

}]);